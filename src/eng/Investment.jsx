  ...
  return <section style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
    <H1 kicker="where the engineering days actually went" title="Investment Mix" right={<div style={{ display: 'flex', gap: 8, alignItems: 'center' }}>
      <div style={{ display: 'flex', gap: 2, padding: 3, borderRadius: 9, background: 'rgba(10,15,22,0.6)' }}>
        {[['pts', 'Story points'], ['days', 'Active days']].map(([v, l]) => <button key={v} onClick={() => setUnit(v)} style={{ padding: '6px 12px', border: 'none', borderRadius: 7, cursor: 'pointer', font: `600 12px ${BODY}`, background: unit === v ? 'rgba(142,200,255,0.2)' : 'transparent', color: unit === v ? '#dbeafc' : '#7f8ea1' }}>{l}</button>)}
      </div>
    ...
    <Card>
      ...
      {!months.length ? <Empty text="Nothing delivered yet in the last 6 months." /> : <>
        ...
        <div style={{ display: 'flex', gap: 14, marginTop: 10, flexWrap: 'wrap' }}>
          {keys.map(k => <span key={k} style={{ display: 'flex', alignItems: 'center', gap: 6, font: `400 11px ${BODY}`, color: '#9fb0c2' }}>
    ...
    {pick && <DataTable title={`${LABEL[pick.bucket]} · ${pick.month}`} meta="everything delivered that month — the ticket list behind the slice" minWidth={640} pageSize={8}
      ...
      columns={[
        ...
        { key: 'type', label: 'Type', width: '92px', sort: r => r.type, render: r => <span style={{ font: `500 11px ${BODY}`, color: r.isBug ? RED : '#93a6bb' }}>{r.type}</span> },
        { key: 'summary', label: 'Summary', width: '2fr', sort: r => r.summary, render: r => <span style={{ font: `400 12px ${BODY}`, color: '#c3cfdc', whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis', display: 'block' }}>{r.summary}</span> },
