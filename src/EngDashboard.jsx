  ...
  return <section style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
    ...
    <Card>
      ...
      <div style={{ display: 'flex', gap: 14, marginTop: 8, flexWrap: 'wrap' }}>
        ...
        <span style={{ marginLeft: 'auto', font: `400 10.5px ${MONO}`, color: DIM }}>{points.filter(p => cyc.p85 != null && p.y > cyc.p85).length} tickets above p85 — those are the escalations</span>
      </div>
    </Card>
    ...
    <div style={{ display: 'grid', gridTemplateColumns: '1.1fr 1fr', gap: 12 }}>
      ...
      <Card>
        ...
        <div style={{ display: 'flex', gap: 16, marginTop: 8, flexWrap: 'wrap' }}>
          ...
          <span style={{ marginLeft: 'auto', font: `400 10.5px ${MONO}`, color: GOLD }}>
    ...
    <div style={{ ...PANEL, padding: '14px 16px' }}>
      <div style={{ display: 'flex', alignItems: 'baseline', justifyContent: 'space-between', marginBottom: 12, flexWrap: 'wrap', gap: 8 }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
          <span style={{ font: `400 10.5px ${MONO}`, color: DIM }}>{active.length} active</span>
          {activePage.pages > 1 && <span style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
            <span style={{ font: `400 10.5px ${MONO}`, color: DIM }}>{activePage.page + 1}/{activePage.pages}</span>
    ...
    {!grp ? <Card><Empty text="No dev-team tickets in any sprint." /></Card> : <>
      <div style={{ display: 'grid', gridTemplateColumns: `repeat(${stats.length},1fr)`, gap: 8 }}>
        {stats.map((s, i) => <div key={i} style={{ ...PANEL, padding: '11px 13px' }}>
          ...
          <div style={{ font: `400 8.5px ${MONO}`, color: DIM, textTransform: 'uppercase', letterSpacing: '0.04em' }}>{s[0]}</div></div>)}
      </div>
      <div style={{ ...PANEL, padding: '14px 16px' }}>
        <div style={{ font: `400 10.5px ${MONO}`, color: DIM, marginBottom: 10 }}>{items.length} ticket{items.length === 1 ? '' : 's'} · the green/gold bar on every card is MINE vs WAITING · click for full detail</div>
        ...
        {['ac', 'tests'].map(kind => {
          ...
          return <Sec key={kind} title={artMeta[kind].title} right={<div style={{ display: 'flex', gap: 6 }}>
            ...
            {editing ? <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
              ...
              <div style={{ font: `400 9.5px ${MONO}`, color: DIM, marginTop: 8 }}>{a.edited ? 'hand-edited' : `generated by ${a.model || 'claude'}`} · {fdate(a.at)}</div>
            </>}
          </Sec>
        })}
      </>}
