...
const KIND = {
  ...
  command: { color: 'var(--amber)', label: 'command' },
  agent: { color: 'var(--blue)', label: 'agent' },
  mcp: { color: 'var(--green)', label: 'mcp server' },
}
  ...
  const node = n => {
    ...
    return (
      <g key={n.id} onClick={() => setSel(isSel ? null : n.id)} style={{ cursor: 'pointer', opacity: on ? 1 : 0.14, transition: 'opacity .2s' }}>
        ...
        {n.kind === 'agent'
          ? <rect x={p.x - r} y={p.y - r} width={r * 2} height={r * 2} rx={5} strokeWidth={isSel ? 2.5 : 1.4}  style={{ fill: (c), stroke: (isSel ? 'var(--text-primary)' : 'var(--bg-surface-active)') }} />
          : n.kind === 'entry'
            ? <path d={`M ${p.x} ${p.y - 14} L ${p.x + 14} ${p.y} L ${p.x} ${p.y + 14} L ${p.x - 14} ${p.y} Z`} strokeWidth={isSel ? 2.5 : 1.4}  style={{ fill: (c), stroke: (isSel ? 'var(--text-primary)' : 'var(--bg-surface-active)') }} />
            : <circle cx={p.x} cy={p.y} r={r} strokeWidth={n.kind === 'command' ? 2.2 : isSel ? 2.5 : 1.4} strokeDasharray={n.ghost ? '3 3' : 'none'}  style={{ fill: (n.kind === 'command' ? 'transparent' : c), stroke: (n.kind === 'command' ? c : isSel ? 'var(--text-primary)' : 'var(--bg-surface-active)') }} />}
        <text x={p.x + (n.kind === 'entry' ? 0 : r + 6)} y={n.kind === 'entry' ? p.y + 28 : p.y + 4} textAnchor={n.kind === 'entry' ? 'middle' : 'start'} style={{ font: `500 10px ${MONO}`, fill: isSel ? 'var(--text-primary)' : 'var(--text-secondary)' }}>{n.name.length > 22 ? n.name.slice(0, 21) + '…' : n.name}</text>
        {deadEnds.includes(n.id) && <circle cx={p.x - r - 3} cy={p.y - r - 3} r={3.2} style={{ fill: 'var(--red)' }}><title>dead end — never reached in any session</title></circle>}
      </g>
    )
  }
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
      ...
      <div style={{ position: 'relative', ...PANEL, animation: 'fadeUp .5s ease both' }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'baseline', marginBottom: 6 }}>
          <div style={{ font: `600 14px ${HEAD}` }}>Orchestration topology <span style={{ font: `400 11px ${MONO}`, color: 'var(--text-secondary)' }}>{mode === 'Observed flow' ? 'what actually ran, weighted by session frequency' : "what's wired up in skill/agent definitions"}</span></div>
          <div style={{ display: 'flex', alignItems: 'center', gap: 14, font: `400 10px ${MONO}`, color: 'var(--text-secondary)' }}>
            ...
            <span style={{ display: 'flex', alignItems: 'center', gap: 5 }}><span style={{ width: 8, height: 8, borderRadius: '50%', background: 'var(--red)' }} />dead end</span>
          </div>
        </div>
        <svg ref={svgRef} viewBox={`0 0 900 ${view.H}`} style={{ width: '100%', height: 'auto', display: 'block' }}>
          {view.vEdges.map((e, i) => {
            ...
            return (
              <path key={i} d={`M ${p.x} ${p.y} C ${mx} ${p.y}, ${mx} ${t.y}, ${t.x} ${t.y}`} fill="none"
                ...
                style={{ stroke: (KIND[nodeById[e.to]?.kind || 'skill'].color), animation: 'dash 1.2s linear infinite', opacity: edgeOn(e) ? (e.trigger ? 0.14 : 0.42) : 0.04, transition: 'opacity .2s' }}>
          ...
          {mode === 'Observed flow' && pulses.map((e, i) => {
            ...
            return (
              <circle key={'p' + i} r="3.2" style={{ fill: (c), filter: `drop-shadow(0 0 5px ${c})` }}>
        ...
        <div style={{ position: 'absolute', bottom: 14, left: 20, font: `400 10px ${MONO}`, color: 'var(--text-tertiary)' }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 16 }}>
        <div style={{ ...PANEL, animation: 'fadeUp .5s ease both', animationDelay: '.1s' }}>
          <div style={{ font: `600 14px ${HEAD}`, marginBottom: 10 }}>Most-traveled path <span style={{ font: `400 11px ${MONO}`, color: 'var(--text-secondary)' }}>observed</span></div>
          {trail.length ? (
            <div style={{ display: 'flex', flexWrap: 'wrap', alignItems: 'center', gap: 6 }}>
              {trail.map((id, i) => (
                <React.Fragment key={id}>
                  {i > 0 && <span style={{ color: 'var(--text-tertiary)' }}>→</span>}
                  <span onClick={() => setSel(id)} style={{ font: `500 11px ${MONO}`, padding: '3px 9px', borderRadius: 6, cursor: 'pointer', background: 'var(--bg-surface-hover)', color: KIND[nodeById[id]?.kind || 'skill'].color }}>{nodeById[id]?.name || id}</span>
                </React.Fragment>
              ))}
            </div>
          ) : <div style={{ font: `400 11px ${MONO}`, color: 'var(--text-tertiary)' }}>no observed invocations yet</div>}
        </div>
        <div style={{ ...PANEL, animation: 'fadeUp .5s ease both', animationDelay: '.15s' }}>
          <div style={{ font: `600 14px ${HEAD}`, marginBottom: 10 }}>Dead ends <span style={{ font: `400 11px ${MONO}`, color: 'var(--text-secondary)' }}>{deadEnds.length} never reached</span></div>
          <div style={{ maxHeight: 180, overflowY: 'auto', display: 'flex', flexWrap: 'wrap', gap: 5 }}>
            {deadEnds.slice(0, 60).map(id => (
              <span key={id} onClick={() => { setQ(nodeById[id]?.name || ''); setSel(id) }} style={{ font: `400 11px ${MONO}`, padding: '2px 8px', borderRadius: 6, cursor: 'pointer', background: 'var(--red-bg)', color: 'var(--red)', border: '1px solid var(--red)' }}>{nodeById[id]?.name || id}</span>
            ))}
            {deadEnds.length === 0 && <span style={{ font: `400 11px ${MONO}`, color: 'var(--green)' }}>✓ everything defined has been used</span>}
          </div>
        </div>
        <div style={{ ...PANEL, animation: 'fadeUp .5s ease both', animationDelay: '.2s' }}>
          <div style={{ font: `600 14px ${HEAD}`, marginBottom: 10 }}>Cycles <span style={{ font: `400 11px ${MONO}`, color: 'var(--text-secondary)' }}>in defined wiring</span></div>
          {cycles.length ? cycles.map(c => (
            <div key={c} style={{ font: `400 11px ${MONO}`, color: 'var(--accent-light)', padding: '4px 0', borderBottom: '1px solid var(--border-subtle)' }}>↻ {c.replace(/(skill|command|agent|mcp|entry):/g, '')}</div>
          )) : <div style={{ font: `400 11px ${MONO}`, color: 'var(--green)' }}>✓ no cycles detected</div>}
        </div>
      </div>
    </div>
  )
