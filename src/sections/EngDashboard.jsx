...
import {
  ...
  Card, CardHead, H1, Empty, Legend, Spinner, Kpi, MiniStat, DataTable, Pager, usePaged, pagerBtn,
...
const stageBudget = (status, pts) => {
  ...
  if (n === 'in code review' || n === 'design qa' || n === 'ready for qa' || n === 'in qa (dev)' || n === 'in qa') return 1
...
function useMe(members) {
  ...
  const resolved = useMemo(() => {
    if (!me) return null
    const m = members.find(x => x.id === me.accountId) || members.find(x => (x.email || '').toLowerCase() === (me.email || '').toLowerCase())
    return { ...me, name: m?.name || '' }
  }, [me, members])
  const setGhHandle = v => { localStorage.setItem('eng.me.gh', v); setGh(v) }
  return [resolved, setGhHandle]
}

export default function EngDashboard({ onExit }) {
  const [url, setUrl] = useUrlState({ route: 'queue', project: '', win: '30d', from: '', to: '', member: '', ticket: '', mine: '' })
  ...
  const selectProject = key => { setUrl({ project: key, member: '' }); load(false, key) }
  ...
  const shell = children => (
    <div style={{ minHeight: '100vh', color: 'var(--text-primary)', fontFamily: BODY, background: 'radial-gradient(950px 440px at 18% -10%,var(--bg-surface-active),transparent 60%),radial-gradient(760px 400px at 104% -4%,var(--blue-bg),transparent 55%),var(--bg-base)' }}>
      <TopBar team={S?.team} projects={projects} project={project} onProject={selectProject} nav={NAV}
        ...
        me={me} setGh={setGh} onExit={onExit} onPalette={() => setPalette(true)} queueN={(S?.triage || []).length} />
      <div style={{ position: 'relative', maxWidth: 1320, margin: '0 auto', padding: '14px 22px 64px', display: 'flex', flexDirection: 'column', gap: 14 }}>
        {S?.available && <Provenance snap={S} onRefresh={() => load(true)} busy={busy} />}
        {children}
        {busy && <LoadingOverlay />}
      </div>
      {config && <ProjectConfig mode={config.mode} project={config.mode === 'edit' ? projects.find(p => p.key === config.key) : null} onClose={() => setConfig(null)} onSaved={list => { setProjects(list); setConfig(null) }} onSelect={selectProject} />}
      {S?.available && <CmdK snap={S} open={palette} setOpen={setPalette} routes={NAV.map(n => [n[0], n[1]])} onRoute={r => setUrl({ route: r })} onOpenTicket={k => setUrl({ route: 'sprint', ticket: k })} />}
  ...
  if (!S.available) return shell(<NotWired reason={err} onSaved={() => load(false)} onAddProject={() => setConfig({ mode: 'new' })} />)

  const issues = S.issues || [], prs = S.prs || []
  ...
  const common = { snap: S, S, issues, prs, members, win, shipped, active, prsFor, onOpenTicket: openTicket, reload: r => load(!!r) }
  ...
  return shell(
    route === 'queue' ? <AttentionQueue snap={S} me={me} mine={url.mine === '1'} setMine={v => setUrl({ mine: v ? '1' : '' })} project={project} onOpenTicket={openTicket} reload={() => load(true)} />
      ...
      : route === 'investment' ? <Investment snap={S} issues={issues} onOpenTicket={openTicket} />
      : route === 'sprints' ? <Predictability snap={S} onOpenTicket={openTicket} />
      : route === 'epics' ? <Epics snap={S} reload={r => load(!!r)} onOpenTicket={openTicket} />
      : route === 'ci' ? <CIHealth snap={S} />
      : route === 'projects' ? <Compare snap={S} win={win} onProject={k => { selectProject(k); setUrl({ route: 'queue', project: k }) }} />
      : route === 'load' ? <Load snap={S} />
      : route === 'sprint' ? <Sprint {...common} open={url.ticket} setOpen={k => setUrl({ ticket: k || '' })} member={url.member} setMember={m => setUrl({ member: m })} />
      : route === 'members' ? <Members {...common} member={url.member || members[0]?.id} setMember={m => setUrl({ member: m })} me={me} onExport={() => setUrl({ route: 'export' })} />
      : route === 'okrs' ? <Okrs {...common} />
      : <Export snap={S} win={win} me={me} />
  )
}

// ---------------- chrome ----------------
function TopBar({ team, projects, project, onProject, onAdd, onEdit, route, setRoute, nav, url, setUrl, win, sprints, me, setGh, onExit, onPalette, queueN }) {
    ...
    <div style={{ position: 'sticky', top: 0, zIndex: 20, background: 'linear-gradient(180deg,var(--bg-surface),var(--bg-base))', borderBottom: '1px solid var(--border-default)' }}>
      <div style={{ display: 'flex', alignItems: 'center', gap: 14, padding: '10px 22px 0', maxWidth: 1320, margin: '0 auto' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 10, flexShrink: 0 }}>
          ...
          <div>
            <div style={{ font: `700 14px ${HEAD}`, letterSpacing: '-0.01em', color: 'var(--text-primary)', lineHeight: 1 }}>Engineering Metrics</div>
            <div style={{ font: `500 10px ${MONO}`, letterSpacing: '0.08em', color: 'var(--text-secondary)', marginTop: 3, textTransform: 'uppercase', height: 11 }}>{activeName}</div>
          </div>
        </div>
        <div style={{ marginLeft: 'auto', display: 'flex', alignItems: 'center', gap: 8, flexWrap: 'wrap', justifyContent: 'flex-end' }}>
          <TimeLens value={url.win} from={url.from} to={url.to} onChange={setUrl} sprints={sprints} resolved={win} />
          <div style={{ display: 'flex', alignItems: 'center', gap: 4, padding: '2px 2px 2px 4px', borderRadius: 6, background: 'var(--bg-surface-hover)', border: '1px solid var(--border-default)' }}>
            <span style={{ font: `500 9px ${MONO}`, letterSpacing: '0.06em', color: 'var(--text-secondary)', textTransform: 'uppercase', paddingLeft: 4 }}>project</span>
            ...
            {project && project !== 'all' && <button onClick={onEdit} title="Configure this project" style={{ width: 24, height: 24, borderRadius: 7, border: '1px solid var(--border-default)', background: 'var(--bg-surface)', color: 'var(--text-primary)', cursor: 'pointer', fontSize: 12, padding: 0 }}>⚙</button>}
            <button onClick={onAdd} title="Add a project" style={{ width: 24, height: 24, borderRadius: 7, border: '1px solid var(--border-default)', background: 'var(--bg-surface)', color: 'var(--text-primary)', cursor: 'pointer', font: `600 14px ${BODY}`, padding: 0 }}>+</button>
          </div>
          ...
          <button onClick={onExit} title="back to Claude Code dashboard" style={{ cursor: 'pointer', font: `500 12px ${BODY}`, color: 'var(--text-secondary)', padding: '6px 10px', borderRadius: 8, border: '1px solid var(--border-default)', background: 'transparent' }}>← Claude</button>
        </div>
      </div>
      {idOpen && <div style={{ maxWidth: 1320, margin: '8px auto 0', padding: '0 22px' }}>
        <div style={{ ...PANEL, padding: '10px 14px', display: 'flex', alignItems: 'center', gap: 10, flexWrap: 'wrap' }}>
          <span style={{ font: `400 12px ${BODY}`, color: 'var(--text-secondary)' }}>JIRA: <b style={{ color: HI }}>{me?.name || me?.email || '—'}</b> · GitHub login (used to resolve “Mine” on PR rows):</span>
      ...
      <nav style={{ display: 'flex', alignItems: 'center', gap: 2, padding: '8px 22px 6px', maxWidth: 1320, margin: '0 auto', overflowX: 'auto' }}>
        {nav.map(([id, label, icon]) => {
          const a = route === id
          return <button key={id} onClick={() => setRoute(id)} style={{ display: 'flex', alignItems: 'center', gap: 6, padding: '6px 10px', border: 'none', borderRadius: 6, cursor: 'pointer', font: `500 13px ${BODY}`, background: a ? 'var(--bg-surface-active)' : 'transparent', color: a ? 'var(--text-primary)' : 'var(--text-secondary)', whiteSpace: 'nowrap' }}>
            <span style={{ fontSize: 11, opacity: 0.9 }}>{icon}</span>{label}
            {id === 'queue' && queueN > 0 && <span style={{ font: `700 9px ${MONO}`, padding: '1px 5px', borderRadius: 4, background: 'var(--red-bg)', color: RED }}>{queueN}</span>}
          </button>
        })}
      </nav>
    </div>
...
function Overview({ S, issues, shipped, active, members, prs, win, onOpenTicket }) {
  ...
  const kpis = [
    { label: 'Cycle time', value: cyc.n ? fx(cyc.p50) + 'd' : '—', color: BB, sub: spread(cyc), n: cyc.n, thin: cyc.n < MIN_N, delta: statDelta(cyc, cycPrev), arr: shipped.map(i => i.delivery).filter(Boolean) },
    { label: 'Lead time', value: lead.n ? fx(lead.p50) + 'd' : '—', color: STEEL, sub: `${spread(lead)} · created → live (the queue half)`, n: lead.n, thin: lead.n < MIN_N, arr: shipped.map(i => i.leadDays) },
    { label: '1st review', value: R.firstReview?.p50 == null ? '—' : fx(R.firstReview.p50) + 'd', color: PURPLE, sub: `p90 ${fx(R.firstReview?.p90)}d · from the REQUEST, not the push`, n: prs.filter(p => p.firstReviewFromRequestDays != null).length },
    { label: 'Merge time', value: R.mergeTime?.p50 == null ? '—' : fx(R.mergeTime.p50) + 'd', color: GREEN, sub: `p90 ${fx(R.mergeTime?.p90)}d`, n: prs.filter(p => p.mergeDays != null).length },
    { label: 'QA cycles', value: qa.n ? fx(qa.p50) : '—', color: GOLD, sub: `p90 ${fx(qa.p90)} re-test loops`, n: qa.n, thin: qa.n < MIN_N, arr: shipped.map(i => i.qaCycles) },
    { label: 'Escape rate', value: escRate == null ? '—' : fx(escRate, 0) + '%', color: escRate > 10 ? RED : GREEN, sub: 'bugs that reached production' },
  ]
  const chips = [
  ...
  const stages = ['In Progress', 'In Code Review', 'Ready for QA', 'In QA (Dev)', 'QA Blocked'].map(st => {
    ...
    const s = stat(vals)
    const budgets = issues.filter(i => (i.daysIn?.[st] || 0) > 0.01).map(i => stageBudget(st, i.pts)).filter(Boolean)
  ...
  for (let k = 5; k >= 0; k--) {
    ...
    trend.push({ label: MONTHS[d.getMonth()].slice(0, 3), cyc: set.length ? of(set, i => pos(i.delivery)).p50 : null, lead: set.length ? of(set, i => pos(i.leadDays)).p50 : null, n: set.length })
  }
  ...
  return <section style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
    <H1 kicker={win.label} title="Team Overview" right={<div style={{ display: 'flex', gap: 8 }}>
      {chips.map((c, i) => <div key={i} style={{ display: 'flex', alignItems: 'center', gap: 7, padding: '7px 12px', borderRadius: 6, background: 'var(--bg-surface)', border: '1px solid var(--border-default)' }}>
        <span style={{ font: `700 14px ${HEAD}`, color: c.c }}><CountUp value={Number(c.v)} /></span><span style={{ font: `400 11px ${BODY}`, color: 'var(--text-secondary)' }}>{c.l}</span></div>)}
    </div>} />

    <div style={{ display: 'grid', gridTemplateColumns: 'repeat(6,1fr)', gap: 10 }}>
      {kpis.map((k, i) => <Kpi key={i} {...k} onCopy={k.arr ? () => copy(JSON.stringify(k.arr), 'k' + i) : undefined} />)}
    </div>

    <Card>
      <CardHead title="Cycle time — every ticket, not an average" meta={`one dot per shipped ticket · coloured by type · click a dot above p85 · ${cyc.n} tickets`}
        right={<button style={miniBtn} onClick={() => copy(JSON.stringify(shipped.map(i => ({ key: i.key, cycle: i.delivery, live: i.liveAt, type: i.type })), null, 2), 'sc')}>{copied === 'sc' ? '✓ copied' : '{ }'}</button>} />
      <Scatter points={points} p85={cyc.p85} p50={cyc.p50} onPick={p => onOpenTicket(p.key)} />
      <div style={{ display: 'flex', gap: 14, marginTop: 8, flexWrap: 'wrap' }}>
        {[...new Set(shipped.map(i => i.type))].slice(0, 6).map(t => <Legend key={t} c={typeColor(t)} label={t} />)}
        <span style={{ marginLeft: 'auto', font: `400 11px ${MONO}`, color: DIM }}>{points.filter(p => cyc.p85 != null && p.y > cyc.p85).length} tickets above p85 — those are the escalations</span>
      </div>
    </Card>
    ...
    <div style={{ display: 'grid', gridTemplateColumns: '1.1fr 1fr', gap: 12 }}>
      <Card>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 11 }}>
          {stages.map(({ st, s, budget, over }) => <div key={st}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 3 }}>
              <span style={{ width: 118, flexShrink: 0, font: `500 12px ${BODY}`, color: 'var(--text-secondary)' }}>{st}</span>
              <div style={{ flex: 1, position: 'relative', height: 10, borderRadius: 5, background: 'var(--bg-surface-hover)', overflow: 'visible' }}>
              ...
              <span style={{ width: 96, textAlign: 'right', font: `600 11px ${MONO}`, color: s.n < MIN_N ? 'var(--text-secondary)' : HI }}>{fx(s.p50)} / {fx(s.p90)}d</span>
            </div>
        ...
        <div style={{ marginTop: 12, paddingTop: 10, borderTop: '1px solid var(--border-default)' }}>
          <div style={{ font: `600 10px ${MONO}`, textTransform: 'uppercase', letterSpacing: '0.05em', color: 'var(--text-secondary)', marginBottom: 6 }}>Mine vs the queue — shipped in this window</div>
          ...
          <div style={{ font: `400 10px ${MONO}`, color: DIM, marginTop: 5 }}>Cycle time is presented everywhere as a property of the developer. {activeSum + waitSum ? Math.round(waitSum / (activeSum + waitSum) * 100) : 0}% of it was queue.</div>
        </div>
      </Card>

      <Card>
        <CardHead title="Lead vs cycle — 6 months" meta="p50 · the gap is time it sat waiting on us" />
        <Lines labels={trend.map(t => t.label)} yFmt={v => v + 'd'}
          series={[
            { label: 'lead (created → live)', color: PURPLE, values: trend.map(t => ({ y: t.lead, note: `n=${t.n}` })) },
            { label: 'cycle (In Progress → live)', color: BB, values: trend.map(t => ({ y: t.cyc, note: `n=${t.n}` })) },
          ]} />
    ...
    <div style={{ ...PANEL, padding: '14px 16px' }}>
      <div style={{ display: 'flex', alignItems: 'baseline', justifyContent: 'space-between', marginBottom: 12, flexWrap: 'wrap', gap: 8 }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
          ...
          {activePage.pages > 1 && <span style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
            <span style={{ font: `400 11px ${MONO}`, color: DIM }}>{activePage.page + 1}/{activePage.pages}</span>
      ...
      {active.length === 0 ? <Empty text="Nothing in an active status right now." /> : <ColumnsBoard items={activePage.slice} onOpen={i => onOpenTicket(i.key)} />}
    </div>
  </section>
}
...
function IssueTip({ i, children }) {
  ...
  return <span onMouseEnter={e => { const r = e.currentTarget.getBoundingClientRect(); setPos({ left: r.left, top: r.top, bottom: r.bottom }) }} onMouseLeave={() => setPos(null)} style={{ position: 'relative', cursor: 'help' }}>
    {children}
    {pos && <span style={box}>
      <div style={{ font: `600 10px ${MONO}`, letterSpacing: '0.06em', textTransform: 'uppercase', color: 'var(--text-secondary)', marginBottom: 6 }}>{i.key} · in {i.status}</div>
      <TipRow label="In this column" value={`${fx(i.inCurrent)}d`} />
      <TipRow label="Estimate" value={i.estAcc == null ? `${i.pts || 0}pt` : `${i.pts}pt · ${fx(i.estAcc, 0)}% accurate`} c={i.estAcc == null ? 'var(--text-primary)' : accColor(i.estAcc)} />
      ...
      {rec && <div style={{ font: `400 10px ${MONO}`, color: DIM, margin: '5px 0 6px' }}>{rec.atRisk ? `${fx(Math.abs(rec.remaining))}d over budget` : `${fx(rec.remaining)}d of ${fx(rec.budget)}d budget left`}</div>}
      <Split active={i.activeDays} wait={i.waitDays} showLabels />
    </span>}
  </span>
}
const TipRow = ({ label, value, c = 'var(--text-primary)' }) => <div style={{ display: 'flex', justifyContent: 'space-between', gap: 10, padding: '2px 0' }}>
...
function BoardCard({ i, onOpen }) {
  ...
  return <IssueTip i={i}>
    <div className={onOpen ? 'press' : undefined} onClick={onOpen ? () => onOpen(i) : undefined} style={{ padding: '9px 10px', borderRadius: 6, background: 'var(--bg-surface)', border: `1px solid ${c}`, display: 'flex', flexDirection: 'column', gap: 6, cursor: onOpen ? 'pointer' : undefined }}>
      ...
      {i.parent && <div style={{ display: 'flex', alignItems: 'center', gap: 4, minWidth: 0 }}>
        ...
        <TicketLink i={{ key: i.parent.key, host: i.host }} color="var(--text-secondary)" style={{ font: `500 9px ${MONO}`, borderBottom: 'none' }}>{i.parent.key}</TicketLink>
        <span style={{ font: `400 9px ${BODY}`, color: DIM, whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>{i.parent.summary}</span>
      </div>}
      <div style={{ font: `500 12px ${BODY}`, color: 'var(--text-primary)', lineHeight: 1.35, display: '-webkit-box', WebkitLineClamp: 2, WebkitBoxOrient: 'vertical', overflow: 'hidden' }}>{i.summary}</div>
      ...
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
        <span style={{ font: `400 10px ${MONO}`, color: 'var(--text-secondary)' }}>{i.sprint?.name || 'no sprint'}</span>
        <span style={{ font: `600 9px ${MONO}`, color: 'var(--text-secondary)', padding: '1px 6px', borderRadius: 4, background: 'var(--bg-surface-active)' }}>{i.pts || 0} pt</span>
      </div>
    </div>
  </IssueTip>
}
const RoleChip = ({ label, name, c }) => <span style={{ display: 'inline-flex', alignItems: 'center', gap: 4, font: `500 9px ${MONO}`, padding: '1px 6px', borderRadius: 5, background: 'var(--bg-base)', border: '1px solid var(--border-default)' }}>
  <span style={{ color: c, fontWeight: 700 }}>{label}</span><span style={{ color: name === '—' ? 'var(--text-tertiary)' : 'var(--text-secondary)' }}>{name}</span></span>
  ...
  return <div style={{ display: 'flex', gap: 10, overflowX: 'auto', paddingBottom: 6 }}>
    {cols.map(col => {
      ...
      return <div key={col} style={{ flex: `0 0 ${minCol}px`, width: minCol, display: 'flex', flexDirection: 'column', gap: 8 }}>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 8, minHeight: 40, padding: 6, borderRadius: 6, background: 'var(--bg-base)', border: '1px solid var(--border-subtle)' }}>
          {its.map(i => <BoardCard key={i.key} i={i} onOpen={onOpen} />)}
          {its.length === 0 && <div style={{ padding: '10px 6px', font: `400 11px ${MONO}`, color: 'var(--bg-surface-active)', textAlign: 'center' }}>—</div>}
        </div>
      </div>
    })}
  </div>
...
function sprintStats(items) {
  ...
  return [
    ...
    ['At risk', String(items.filter(i => i.rec?.atRisk).length), items.some(i => i.rec?.atRisk) ? GOLD : 'var(--text-secondary)'],
  ]
}
...
function Sprint({ issues, members, open, setOpen, member, setMember }) {
  ...
  return <section style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
    <H1 kicker="sprint board · dev team" title="Board" right={<div style={{ display: 'flex', gap: 8 }}>
      <select value={who} onChange={e => setMember(e.target.value === 'all' ? '' : e.target.value)} style={{ ...sel, padding: '9px 14px', font: `600 13px ${BODY}` }}>
    ...
    {!grp ? <Card><Empty text="No dev-team tickets in any sprint." /></Card> : <>
      <div style={{ display: 'grid', gridTemplateColumns: `repeat(${stats.length},1fr)`, gap: 8 }}>
        {stats.map((s, i) => <div key={i} style={{ ...PANEL, padding: '11px 13px' }}>
          ...
          <div style={{ font: `400 9px ${MONO}`, color: DIM, textTransform: 'uppercase', letterSpacing: '0.04em' }}>{s[0]}</div></div>)}
      </div>
      <div style={{ ...PANEL, padding: '14px 16px' }}>
        <div style={{ font: `400 11px ${MONO}`, color: DIM, marginBottom: 10 }}>{items.length} ticket{items.length === 1 ? '' : 's'} · the green/gold bar on every card is MINE vs WAITING · click for full detail</div>
...
function Members({ issues, prs, members, member, setMember, prsFor, win, me, onExport }) {
  ...
  return <section style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
    ...
    <div style={{ font: `400 11px ${BODY}`, color: DIM, lineHeight: 1.6 }}>
      Every number here is a JIRA ticket or a GitHub PR the subject can open about themselves, scoped to <b style={{ color: 'var(--text-secondary)' }}>{win.label}</b>. The radar is <b style={{ color: 'var(--text-secondary)' }}>relative</b> — each axis is this person's rank inside this team over this window, not an absolute score — and it is flagged low-confidence below {MIN_N} shipped tickets. Cards are alphabetical, because a sort order is a scoreboard. Read it to coach and unblock, never to rank.
    </div>
    ...
    <DataTable title={`Work items · ${cur?.name || '—'}`} minWidth={620} pageSize={10} rows={mine} getKey={r => r.key} initialSort={{ key: 'status', dir: 1 }} raw={mine}
      columns={[
        ...
        { key: 'summary', label: 'Summary', width: '1.7fr', sort: r => r.summary, render: r => <span style={{ font: `400 12px ${BODY}`, color: 'var(--text-secondary)', whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis', display: 'block' }}>{r.summary}</span> },
    ...
    <DataTable title="Pull requests" meta="linked to their tickets · checks on every row" minWidth={700} pageSize={8} rows={myPrs} getKey={p => p.num} initialSort={{ key: 'merge', dir: -1 }} raw={myPrs}
      columns={[
        ...
        { key: 'title', label: 'Title', width: '1.9fr', sort: p => p.title, render: p => <span style={{ font: `400 12px ${BODY}`, color: 'var(--text-secondary)', whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis', display: 'block' }}><PRLink pr={p}>#{p.num}</PRLink> {p.title}</span> },
        { key: 'cycles', label: 'Rounds', width: '72px', align: 1, sort: p => p.cycles, render: p => <span style={{ font: `600 12px ${MONO}`, color: p.cycles > 1 ? GOLD : 'var(--text-secondary)' }}>{p.cycles}</span> },
...
function insightsFor(i) {
  ...
  if (i.waitDays > i.activeDays && i.waitDays > 1) out.push({ c: GOLD, t: `${fx(i.waitDays)}d of the ${fx(i.activeDays + i.waitDays)}d elapsed was WAITING (review / QA), not development` })
...
function TicketDetail({ issue: i, onClose }) {
  ...
  const metrics = [['Cycle', i.delivery != null ? fx(i.delivery) + 'd' : '—', BB], ['Lead', i.leadDays != null ? fx(i.leadDays) + 'd' : '—', STEEL], ['Mine', fx(i.activeDays) + 'd', GREEN],
    ['Waiting', fx(i.waitDays) + 'd', GOLD], ['QA cycles', String(i.qaCycles), GOLD], ['Est acc', i.estAcc == null ? '—' : fx(i.estAcc, 0) + '%', i.estAcc == null ? 'var(--text-secondary)' : accColor(i.estAcc)]]
    ...
    <div onClick={e => e.stopPropagation()} style={{ width: 560, maxWidth: '96vw', height: '100vh', overflowY: 'auto', background: 'linear-gradient(180deg,var(--bg-surface),var(--bg-base))', borderLeft: '1px solid var(--border-default)', padding: '18px 20px 60px', display: 'flex', flexDirection: 'column', gap: 14 }}>
      <div style={{ display: 'flex', alignItems: 'flex-start', gap: 10 }}>
        <div style={{ flex: 1, minWidth: 0 }}>
          ...
          <div style={{ font: `700 16px ${HEAD}`, color: 'var(--text-primary)', marginTop: 6, lineHeight: 1.3 }}>{i.summary}</div>
        </div>
      ...
      <Sec title="Key data">
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '4px 16px' }}>
          {kd.map(([k, v]) => <div key={k} style={{ display: 'flex', justifyContent: 'space-between', gap: 8, borderBottom: '1px solid var(--border-subtle)', padding: '3px 0' }}>
            <span style={{ font: `400 11px ${BODY}`, color: 'var(--text-secondary)' }}>{k}</span>
            <span style={{ font: `600 11px ${MONO}`, color: 'var(--text-primary)', textAlign: 'right', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{v}</span></div>)}
        </div>
      </Sec>

      <Sec title="Metrics">
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3,1fr)', gap: 8, marginBottom: 12 }}>
          {metrics.map(([l, v, c]) => <div key={l} style={{ padding: '8px 10px', borderRadius: 6, background: 'var(--bg-base)', border: '1px solid var(--border-default)' }}>
            <div style={{ font: `700 16px ${HEAD}`, color: c }}>{v}</div><div style={{ font: `400 9px ${MONO}`, color: DIM, textTransform: 'uppercase' }}>{l}</div></div>)}
        </div>
        <div style={{ marginBottom: 12 }}>
          <Split active={i.activeDays} wait={i.waitDays} height={10} showLabels />
          <div style={{ font: `400 10px ${MONO}`, color: DIM, marginTop: 4 }}>
            {i.waitDays > 0 ? `${Math.round(i.waitDays / (i.activeDays + i.waitDays || 1) * 100)}% of the elapsed working time was queue — review, QA or release — not development.` : 'No waiting time recorded.'}
          </div>
        </div>
        {bars.map(([k, v]) => <div key={k} style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '2px 0' }}>
          <span style={{ width: 120, flexShrink: 0, font: `500 11px ${BODY}`, color: WAITING.includes(k) ? GOLD : 'var(--text-secondary)' }}>{k}</span>
          <div style={{ flex: 1, height: 7, borderRadius: 4, background: 'var(--bg-surface-hover)', overflow: 'hidden' }}><div style={{ height: '100%', width: `${v / barMax * 100}%`, background: colorFor(k), borderRadius: 4 }} /></div>
          <span style={{ width: 40, textAlign: 'right', font: `600 11px ${MONO}`, color: HI }}>{fx(v)}d</span></div>)}
      </Sec>

      <Sec title="Insights">
        <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
          {insightsFor(i).map((s, idx) => <div key={idx} style={{ display: 'flex', gap: 8, alignItems: 'baseline' }}>
            <span style={{ color: s.c, fontSize: 10 }}>●</span><span style={{ font: `400 12px ${BODY}`, color: 'var(--text-secondary)' }}>{s.t}</span></div>)}
        </div>
      </Sec>

      {err && <div style={{ font: `400 12px ${BODY}`, color: RED }}>Failed to load detail: {err}</div>}
      {!d && !err && <div style={{ ...PANEL, padding: '13px 15px', display: 'flex', gap: 8, alignItems: 'center', color: 'var(--text-secondary)' }}><Spinner /><span style={{ font: `500 12px ${BODY}` }}>Loading content, history & PR context…</span></div>}

      {d && <>
        <Sec title="Ticket content">
          {d.description ? <div style={{ font: `400 12px/1.6 ${BODY}`, color: 'var(--text-secondary)', whiteSpace: 'pre-wrap', maxHeight: 260, overflow: 'auto' }}>{d.description}</div> : <Empty text="No description." />}
          {d.comments?.length > 0 && <div style={{ marginTop: 10, display: 'flex', flexDirection: 'column', gap: 8 }}>
            {d.comments.slice(-6).map((c, idx) => <div key={idx} style={{ borderLeft: '2px solid var(--border-default)', paddingLeft: 10 }}>
              <div style={{ font: `600 10px ${MONO}`, color: 'var(--text-secondary)' }}>{c.author} · {fdate(c.at)}</div>
              <div style={{ font: `400 12px/1.5 ${BODY}`, color: 'var(--text-secondary)', whiteSpace: 'pre-wrap' }}>{c.body}</div></div>)}
          </div>}
        </Sec>

        {d.prs?.length > 0 && <Sec title={`Linked PRs (${d.prs.length})`}>
          {d.prs.map(p => <div key={p.num} style={{ padding: '6px 0', borderBottom: '1px solid var(--border-subtle)' }}>
            ...
            <div style={{ font: `400 11px ${BODY}`, color: 'var(--text-secondary)' }}>{p.title}</div></div>)}
        </Sec>}

        <Sec title="History">
          <div style={{ display: 'flex', flexDirection: 'column', gap: 4, maxHeight: 220, overflow: 'auto' }}>
            {!d.history?.length ? <Empty text="No transitions recorded." /> : d.history.map((h, idx) => <div key={idx} style={{ display: 'flex', gap: 8, font: `400 11px ${MONO}` }}>
              <span style={{ color: DIM, flexShrink: 0, width: 104 }}>{fdt(h.at)}</span>
              <span style={{ color: 'var(--text-secondary)' }}>{h.field === 'status' ? '' : h.field + ': '}{h.from ? `${h.from} → ` : ''}<span style={{ color: 'var(--text-primary)' }}>{h.to}</span></span></div>)}
          </div>
        </Sec>
        ...
        {['ac', 'tests'].map(kind => {
          ...
          return <Sec key={kind} title={artMeta[kind].title} right={<div style={{ display: 'flex', gap: 6 }}>
            ...
            {busy === kind && <div style={{ display: 'flex', gap: 8, alignItems: 'center', color: 'var(--text-secondary)' }}><Spinner /><span style={{ font: `500 12px ${BODY}` }}>Asking claude…</span></div>}
...
function Okrs({ S, issues, shipped, prs, win }) {
  ...
  const auto = {
    ...
    estAcc: of(shipped.filter(i => i.estAcc != null), i => i.estAcc).p50,
  ...
  const n = { devTime: shipped.filter(i => i.dev > 0).length, crTime: shipped.filter(i => i.cr > 0).length, estAcc: shipped.filter(i => i.estAcc != null).length, qaCycles: shipped.length, cycle: shipped.filter(i => i.delivery > 0).length, stale: issues.length, firstReview: prs.length, mergeTime: merged.length, reworkRate: issues.length }
  ...
  const objs = (S.okrs?.[quarter] || []).map(o => {
    const measures = o.measures.map(m => {
      ...
      const color = noData || thin ? 'var(--text-secondary)' : pct >= 90 ? GREEN : pct >= 60 ? BB : pct >= 40 ? GOLD : RED
  ...
  return <section style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
    <H1 kicker={`${quarter} · measured over ${win.label}`} title="Objectives & Key Results" right={
      <span style={{ font: `400 11px ${MONO}`, color: GOLD, maxWidth: 380, textAlign: 'right', lineHeight: 1.5 }}>
        Every measure is a MEDIAN over the Time Lens window. The old private Q3/Q4 toggle is gone — it relabelled objectives without moving the data.
      </span>} />
    {objs.map((o, oi) => <div key={oi} style={{ ...PANEL, padding: '18px 20px' }}>
      <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', gap: 18, flexWrap: 'wrap', marginBottom: 14 }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 12, flexShrink: 0 }}>
          <div style={{ textAlign: 'right' }}><div style={{ font: `700 20px ${HEAD}`, color: o.color }}>{o.pct}%</div><div style={{ font: `400 10px ${MONO}`, color: DIM }}>objective</div></div>
          <svg width="60" height="60" viewBox="0 0 60 60"><circle cx="30" cy="30" r="24" fill="none" strokeWidth="6"  style={{ stroke: 'var(--text-secondary)' }} /><circle cx="30" cy="30" r="24" fill="none" strokeWidth="6" strokeLinecap="round" strokeDasharray="150.8" strokeDashoffset={(150.8 * (1 - o.pct / 100)).toFixed(1)} transform="rotate(-90 30 30)"  style={{ stroke: (o.color) }} /></svg>
        </div>
      </div>
      <div style={{ display: 'flex', flexDirection: 'column', gap: 9 }}>
        {o.measures.map((m, mi) => <div key={mi} style={{ padding: '12px 14px', borderRadius: 6, background: 'var(--bg-base)', border: '1px solid var(--border-default)' }}>
          <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', gap: 12, marginBottom: 9 }}>
            <div style={{ minWidth: 0 }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 2 }}>
                ...
                <span style={{ font: `600 8px ${MONO}`, letterSpacing: '0.04em', padding: '1px 6px', borderRadius: 4, background: 'var(--bg-surface-active)', color: 'var(--text-secondary)' }}>{m.src} · n={m.n}</span>
                {m.thin && <span style={{ font: `600 8px ${MONO}`, color: GOLD }}>n&lt;{MIN_N} — not a trend</span>}
              </div>
              <div style={{ font: `400 11px ${MONO}`, color: DIM }}>{m.note}</div>
            </div>
            <div style={{ textAlign: 'right', flexShrink: 0 }}><span style={{ font: `700 14px ${HEAD}`, color: HI }}>{m.curTxt}</span><span style={{ font: `400 11px ${MONO}`, color: DIM }}> / {m.tgtTxt}</span></div>
          </div>
          <div style={{ display: 'flex', alignItems: 'center', gap: 11 }}>
            <div style={{ flex: 1, height: 6, borderRadius: 4, background: 'var(--bg-surface-hover)', overflow: 'hidden' }}><div style={{ height: '100%', width: `${m.pct}%`, borderRadius: 4, background: m.color }} /></div>
...
function ProjectConfig({ mode, project, onClose, onSaved, onSelect }) {
  ...
  const field = (label, key, ph, dis) => <label style={{ display: 'block' }}>
    <div style={{ font: `600 10px ${MONO}`, letterSpacing: '0.05em', textTransform: 'uppercase', color: 'var(--text-secondary)', marginBottom: 5 }}>{label}</div>
    <input value={f[key]} onChange={set(key)} placeholder={ph} disabled={dis} style={{ ...inp, opacity: dis ? 0.5 : 1 }} /></label>
  return <div onClick={onClose} style={{ position: 'fixed', inset: 0, zIndex: 100, background: 'var(--bg-base)', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 20 }}>
    <div onClick={e => e.stopPropagation()} style={{ ...PANEL, width: 500, maxWidth: '94vw', maxHeight: '90vh', overflowY: 'auto', padding: 22 }}>
      <div style={{ font: `700 16px ${HEAD}`, color: 'var(--text-primary)', marginBottom: 4 }}>{isEdit ? `Configure ${project.jiraProjectKey}` : 'Add a project'}</div>
      <div style={{ font: `400 12px ${BODY}`, color: 'var(--text-secondary)', marginBottom: 16 }}>Ties a GitHub repo to a JIRA board and defines the team by role. Persisted server-side.</div>
      ...
      <div style={{ font: `600 10px ${MONO}`, letterSpacing: '0.05em', textTransform: 'uppercase', color: 'var(--text-secondary)', margin: '18px 0 8px' }}>Team members & roles</div>
      <div style={{ display: 'flex', flexDirection: 'column', gap: 7 }}>
        {members.map((m, i) => <div key={i} style={{ display: 'flex', gap: 7 }}>
          ...
          <button onClick={() => setMembers(members.filter((_, j) => j !== i))} style={{ width: 34, flexShrink: 0, borderRadius: 6, border: '1px solid var(--border-default)', background: 'transparent', color: 'var(--text-secondary)', cursor: 'pointer' }}>✕</button>
        </div>)}
      </div>
      <button onClick={() => setMembers([...members, { email: '', role: 'dev' }])} style={{ marginTop: 8, padding: '6px 12px', borderRadius: 8, border: '1px dashed var(--border-default)', background: 'transparent', color: 'var(--text-secondary)', cursor: 'pointer', font: `500 12px ${BODY}` }}>+ Add member</button>
      {err && <div style={{ marginTop: 12, font: `400 12px ${BODY}`, color: RED }}>{err}</div>}
      <div style={{ display: 'flex', justifyContent: 'flex-end', gap: 9, marginTop: 18 }}>
        <button onClick={onClose} style={{ padding: '8px 15px', borderRadius: 6, border: '1px solid var(--border-default)', background: 'transparent', color: 'var(--text-secondary)', cursor: 'pointer', font: `500 13px ${BODY}` }}>Cancel</button>
        <button onClick={submit} disabled={busy || (!isEdit && (!f.jiraProjectKey || !f.githubRepo))} style={{ ...primaryBtn, padding: '8px 15px', fontSize: 13, background: busy ? 'var(--bg-surface-active)' : primaryBtn.background }}>{busy ? 'Saving…' : isEdit ? 'Save changes' : 'Add project'}</button>
      </div>
    </div>
  </div>
}
function CredsForm({ onSaved }) {
  ...
  return <div style={{ display: 'flex', flexDirection: 'column', gap: 9, marginTop: 4 }}>
    <input value={f.email} onChange={e => setF({ ...f, email: e.target.value })} placeholder="atlassian email" style={inp} />
    <input value={f.token} onChange={e => setF({ ...f, token: e.target.value })} placeholder="API token" type="password" style={inp} />
...
function Loading() {
  return <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
    ...
    <div style={{ display: 'grid', gridTemplateColumns: 'repeat(6,1fr)', gap: 10 }}>
      {[0, 1, 2, 3, 4, 5].map(i => <span key={i} className="skel" style={{ height: 78, borderRadius: 6 }} />)}
    </div>
...
function LoadingOverlay() {
  return <div style={{ position: 'absolute', inset: 0, zIndex: 40, background: 'var(--bg-base)', display: 'flex', alignItems: 'flex-start', justifyContent: 'center', paddingTop: 130 }}>
    <div style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '10px 16px', borderRadius: 6, background: 'var(--bg-surface)', border: '1px solid var(--border-default)' }}>
      <Spinner /><span style={{ font: `500 12px ${BODY}`, color: 'var(--text-primary)' }}>Updating…</span>
    </div>
  </div>
}
function NotWired({ reason, onSaved, onAddProject }) {
  return <div style={{ ...PANEL, padding: 28, maxWidth: 560, margin: '40px auto' }}>
    <div style={{ font: `700 16px ${HEAD}`, color: 'var(--text-primary)', marginBottom: 8 }}>Connect JIRA to go live</div>
    ...
    <div style={{ borderTop: '1px solid var(--border-default)', paddingTop: 16, marginTop: 18, display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 12 }}>
      <div style={{ font: `400 12px ${BODY}`, color: 'var(--text-secondary)' }}>No project yet, or need a different board?</div>
