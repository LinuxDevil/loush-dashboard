  ...
  return (
    <div className="hx" style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
      <div className="hx-2a">
        ...
        <div style={{ ...PANEL, display: 'flex', flexDirection: 'column', gap: 12 }}>
          ...
          <div style={{ display: 'flex', flexWrap: 'wrap', gap: 7 }}>
            {doc.inputs.map((inp, i) => {
              ...
              return (
                <span key={i} style={{ display: 'inline-flex', alignItems: 'center', gap: 6, font: `500 11px ${MONO}`, padding: '4px 10px', borderRadius: 7, background: 'var(--bg-surface-hover)', border: `1px solid ${color}44`, color: 'var(--text-secondary)', maxWidth: 280 }} title={inp.value}>
                  {inp.meta?.dataUrl
                  ...
                  <button aria-label="remove input" onClick={() => setDoc(d => ({ ...d, inputs: d.inputs.filter((_, j) => j !== i) }))}
                    style={{ background: 'none', border: 'none', color: 'var(--text-secondary)', cursor: 'pointer', padding: 0 }}>✕</button>
                </span>
              )
            })}
          ...
          {doc.versions?.length > 0 && (
            <div>
              <div style={{ font: `600 10px ${MONO}`, letterSpacing: '0.08em', textTransform: 'uppercase', color: 'var(--text-tertiary)', marginBottom: 6 }}>Revisions</div>
        ...
        <div style={{ ...PANEL, padding: 0, overflow: 'hidden', display: 'flex', flexDirection: 'column' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '14px 20px', borderBottom: '1px solid var(--border-default)' }}>
            ...
            {doc.quality && (() => {
              const s = doc.quality.score, c = s >= 9 ? 'var(--green)' : s >= 7 ? 'var(--amber)' : 'var(--red)'
            ...
            <span style={{ font: `400 11px ${MONO}`, color: 'var(--text-tertiary)' }}>{doc.output ? `~${Math.ceil(doc.output.length / 4)} tok` : ''}</span>
            {doc.quality?.needsGoal && <span style={{ font: `400 11px ${MONO}`, color: 'var(--amber)' }}>add a goal (first text block) to clear 9/10</span>}
          ...
          <pre style={{ margin: 0, padding: '16px 20px', font: `400 12px/1.7 ${MONO}`, color: 'var(--text-secondary)', whiteSpace: 'pre-wrap', overflowY: 'auto', flex: 1, minHeight: 300, maxHeight: 520 }}>
      ...
      <div style={{ ...PANEL }}>
        ...
        {list.map(p => (
          <div key={p.id} style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '9px 4px', borderBottom: '1px solid var(--border-subtle)' }}>
            <span role="button" tabIndex={0} onClick={() => open(p.id)} onKeyDown={e => e.key === 'Enter' && open(p.id)}
              style={{ font: "500 13px var(--body)", color: 'var(--text-primary)', cursor: 'pointer', flex: 1, minWidth: 0, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{p.title}</span>
            {p.tags.map(t => <span key={t} style={{ font: `500 9px ${MONO}`, padding: '1px 7px', borderRadius: 5, background: 'var(--violet-bg)', color: 'var(--violet)' }}>{t}</span>)}
            <span style={{ font: `400 10px ${MONO}`, color: 'var(--text-tertiary)', flexShrink: 0 }}>{p.inputs} inputs · {p.versions} revisions{p.project ? ' · ' + p.project.split('/').pop() : ''} · {fmtDate(p.updatedAt)}</span>
        ...
        {list.length === 0 && <div style={{ font: `400 11px ${MONO}`, color: 'var(--text-tertiary)' }}>no saved prompts yet</div>}
      </div>
    </div>
  )
}
