...
// 094: the same acceptance criteria as tickable, filterable items rather than a markdown blob.
// Parsed server-side so the schema lives in one place; a line that does not fit the schema is
// KEPT as an unstructured item with a reason, because a migration that silently drops a criterion
// loses a requirement nobody will notice is gone.
function StructuredCriteria({ md }) {
  const [parsed, setParsed] = useState(null)
  const [err, setErr] = useState(null)
  useEffect(() => {
    if (!md) return
    setErr(null)
    fetch('/api/criteria/parse', { method: 'POST', headers: { 'content-type': 'application/json' }, body: JSON.stringify({ md }) })
      .then(r => r.json()).then(d => (d.error ? setErr(d.error) : setParsed(d))).catch(e => setErr(String(e)))
  }, [md])
  if (err) return <div className="small" style={{ color: 'var(--red)' }}>structured view unavailable: {err}</div>
  if (!parsed) return null
  const items = parsed.items || []
  const unstructured = items.filter(i => i.kind === 'unstructured')
  return (
    <details style={{ marginTop: 10 }}>
      <summary className="small">structured view — {items.length} item(s){unstructured.length ? `, ${unstructured.length} kept verbatim` : ''}</summary>
      <div style={{ marginTop: 8 }}>
        <CriteriaPanel parsed={parsed} />
        {unstructured.length > 0 && (
          <div className="small" style={{ color: 'var(--amber)', marginTop: 6 }}>
            {unstructured.length} line(s) did not fit the schema and were kept verbatim rather than dropped.
          </div>
        )}
      </div>
    </details>
          ...
          <Sec key={kind} title={META[kind].title} right={
            ...
            {editing ? (
              ...
              <>
                ...
                {kind === 'ac' && <StructuredCriteria md={a.md} />}
