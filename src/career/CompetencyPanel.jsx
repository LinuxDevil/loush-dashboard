...
const EVID = { red: '#f2a2c4', yellow: ACCENT, green: '#5fd39a' }
...
export default function CompetencyPanel() {
  ...
  if (!c) return <div style={{ ...PANEL, color: '#7a716a' }}>Loading…</div>
  ...
  return (
    <div style={{ display: 'grid', gap: 16 }}>
      ...
      <div style={PANEL}>
        ...
        <div style={{ font: `400 11px ${MONO}`, color: '#7a716a', marginBottom: 10 }}>Self-ratings are yours; the evidence tags below are nudges, not proof. No rating cell is coloured by git/PR evidence.</div>
        <label style={{ font: `400 11px ${MONO}`, color: '#9a8f86' }}>Level self-assessed: <input value={c.levelSelfAssessed || ''} onChange={e => setC({ ...c, levelSelfAssessed: e.target.value })} placeholder="e.g. Senior / L5" style={{ font: `500 12px ${MONO}`, background: '#1c1815', color: '#e5dbd2', border: '1px solid #7a716a55', borderRadius: 6, padding: '3px 8px', marginLeft: 6 }} /></label>
        <div style={{ marginTop: 10 }}>
          {areas.map(area => { const rt = c.ratings[area] || {}; return <div key={area} style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '4px 0' }}>
            ...
            {[1, 2, 3, 4, 5].map(n => <button key={n} onClick={() => setRating(area, n)} style={{ width: 26, height: 26, borderRadius: 6, cursor: 'pointer', font: `600 11px ${MONO}`, color: rt.score1to5 === n ? '#0d0b0a' : '#e5dbd2', background: rt.score1to5 === n ? ACCENT : 'transparent', border: `1px solid ${ACCENT}55` }}>{n}</button>)}
            <input value={rt.note || ''} onChange={e => setNote(area, e.target.value)} placeholder="note" style={{ flex: 1, font: `400 11px ${MONO}`, background: '#1c1815', color: '#cbb', border: '1px solid #7a716a33', borderRadius: 6, padding: '3px 8px' }} />
          </div> })}
        </div>
      </div>
      ...
      <div style={PANEL}>
        ...
        <div style={{ font: `400 11px ${MONO}`, color: '#7a716a', marginBottom: 8 }}>One row per line: <code>level | area | expectation</code>. Evidence tags are a nudge only.</div>
        <textarea value={ladderText} onChange={e => setLadderText(e.target.value)} rows={4} placeholder="Senior | Direction | Drives cross-team technical decisions" style={{ width: '100%', boxSizing: 'border-box', font: `400 12px ${MONO}`, background: '#1c1815', color: '#e5dbd2', border: '1px solid #7a716a55', borderRadius: 8, padding: 8 }} />
        ...
        <div style={{ marginTop: 10, display: 'grid', gap: 6 }}>
          {(c.ladder || []).map((r, i) => <div key={i} style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '4px 0', borderTop: '1px solid rgba(255,255,255,0.05)' }}>
            <div style={{ flex: 1, font: `400 12px ${MONO}` }}><b>{r.level}</b> · {r.area} — <span style={{ color: '#9a8f86' }}>{r.expectation}</span></div>
            ...
            <input value={(r.bragLinks || []).join(', ')} onChange={e => setLinks(i, e.target.value)} placeholder="brag links" style={{ width: 120, font: `400 10.5px ${MONO}`, background: '#1c1815', color: '#cbb', border: '1px solid #7a716a33', borderRadius: 6, padding: '2px 6px' }} />
          </div>)}
        </div>
      </div>
