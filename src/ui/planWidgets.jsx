...
export const KIND = { skill: '#d97757', rule: '#e5a03a', mcp: '#3fb96a', tool: '#5eb3f6', agent: '#a78bfa' }
export const STATUS = { ok: '#4fb477', error: '#e06c5a', running: '#e0b341', warn: '#e5a03a', idle: '#8a807a' }
...
export const StatusDot = ({ status, size = 7, title }) => {
  ...
  return <span title={title || status} style={{ width: size, height: size, borderRadius: '50%', background: c, boxShadow: `0 0 5px ${c}80`, display: 'inline-block', flexShrink: 0, animation: status === 'running' ? 'pulse 1.2s ease-in-out infinite' : 'none' }} />
}

const codeBox = { background: 'rgba(0,0,0,0.30)', borderRadius: 8, padding: '7px 9px', font: `400 10.5px ${MONO}`, color: '#c9bfb7', overflowX: 'auto', whiteSpace: 'pre', margin: 0 }
const label = { font: `600 10px ${MONO}`, color: '#a2988f', textTransform: 'uppercase', letterSpacing: 0.4, margin: '12px 0 5px' }
const Pill = ({ children, hue = '#8a807a' }) => <span style={{ font: `500 9.5px ${MONO}`, padding: '2px 7px', borderRadius: 5, color: hue, background: `${hue}18`, border: `1px solid ${hue}40`, whiteSpace: 'nowrap' }}>{children}</span>

export const FilePathChip = ({ path, onClick }) => {
  const s = String(path || ''); const i = s.lastIndexOf('/')
  return (
    <span onClick={onClick} title={s} style={{ font: `500 10px ${MONO}`, cursor: onClick ? 'pointer' : 'default', color: '#c9bfb7', background: `${KIND.tool}14`, border: `1px solid ${KIND.tool}33`, borderRadius: 5, padding: '2px 7px', display: 'inline-flex', gap: 0, maxWidth: '100%', overflow: 'hidden' }}>
      <span style={{ color: '#7a716a', overflow: 'hidden', textOverflow: 'ellipsis', direction: 'rtl' }}>{i > 0 ? s.slice(0, i + 1) : ''}</span>
      <span style={{ color: '#f6efe9', flexShrink: 0 }}>{i >= 0 ? s.slice(i + 1) : s}</span>
    </span>
  )
}
...
function DiffView({ patch, oldStr, newStr }) {
  ...
  const col = { add: STATUS.ok, del: STATUS.error, ctx: '#8a807a', hdr: KIND.agent }
  return (
    <div>
...
export function KVTable({ obj, depth = 0 }) {
  ...
  if (obj == null || typeof obj !== 'object') return <span style={{ font: `400 10.5px ${MONO}`, color: '#c9bfb7' }}>{String(obj)}</span>
  const entries = Array.isArray(obj) ? obj.map((v, i) => [i, v]) : Object.entries(obj)
  if (!entries.length) return <span style={{ font: `400 10.5px ${MONO}`, color: '#7a716a' }}>{Array.isArray(obj) ? '[ ]' : '{ }'}</span>
  return (
    <div style={{ display: 'grid', gridTemplateColumns: 'auto 1fr', gap: '2px 10px', font: `400 10.5px ${MONO}` }}>
      {entries.map(([k, v]) => {
        const nested = v && typeof v === 'object'
        const isOpen = open[k]
        return (
          <React.Fragment key={k}>
            <span style={{ color: '#8a807a', whiteSpace: 'nowrap' }}>{k}</span>
            <span style={{ color: '#c9bfb7', minWidth: 0 }}>
              {!nested ? <span style={{ wordBreak: 'break-word' }}>{typeof v === 'string' && v.length > 120 ? v.slice(0, 120) + '…' : String(v)}</span>
                : depth >= 3 ? <Pill hue="#7a716a">{Array.isArray(v) ? `[${v.length}]` : `{${Object.keys(v).length} keys}`}</Pill>
                  : <span onClick={() => setOpen({ ...open, [k]: !isOpen })} style={{ cursor: 'pointer' }}>
                      ...
                      {isOpen && <div style={{ marginTop: 4, paddingLeft: 8, borderLeft: '1px solid rgba(255,255,255,0.08)' }}><KVTable obj={v} depth={depth + 1} /></div>}
                    </span>}
            </span>
          </React.Fragment>
        )
...
export function ParamView({ tool, params, onOpenFile }) {
  ...
  if (!has) return <div style={{ font: `400 11px ${MONO}`, color: '#7a716a' }}>no parameters</div>
  const pills = []
  const add = (k, v) => { if (v != null && v !== '' && v !== false) pills.push(<Pill key={k}>{k}: {String(v)}</Pill>) }

  switch (tool) {
    case 'Bash':
      return <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>{p.description && <div style={{ font: `400 11px ${HEAD}`, color: '#a2988f' }}>{p.description}</div>}<ShellBlock cmd={p.command} />{(add('timeout', p.timeout), add('run_in_background', p.run_in_background), pills.length ? <div style={{ display: 'flex', gap: 4, flexWrap: 'wrap' }}>{pills}</div> : null)}</div>
    case 'Read':
    ...
    case 'Task': case 'Agent':
      return <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}><div><Pill hue={KIND.agent}>◆ {p.subagent_type || 'agent'}</Pill></div>{p.description && <div style={{ font: `400 11px ${HEAD}`, color: '#a2988f' }}>{p.description}</div>}{p.prompt && <Expandable text={p.prompt} lines={3} mono={false} />}</div>
    default:
...
export function ResultSection({ step }) {
  ...
  const title = <div style={{ ...label, color: err ? STATUS.error : '#a2988f', display: 'flex', gap: 6, alignItems: 'center' }}>{err && <span>▲</span>}Result{err && <span style={{ font: `600 9px ${MONO}` }}>· error</span>}</div>
  if (r && Array.isArray(r.structuredPatch) && r.structuredPatch.length) return <div>{title}<DiffView patch={r.structuredPatch} /></div>
  if (r && (r.stdout || r.stderr)) return <div>{title}{r.stdout ? <Expandable text={r.stdout} lines={4} /> : null}{r.stderr ? <div style={{ marginTop: 4 }}><div style={{ font: `600 9px ${MONO}`, color: STATUS.warn, marginBottom: 2 }}>stderr</div><Expandable text={r.stderr} lines={4} boxStyle={{ color: '#e0b9b3' }} /></div> : null}</div>
  return <div>{title}<Expandable text={step.result} lines={4} boxStyle={err ? { color: '#e0b9b3', background: `${STATUS.error}10` } : undefined} /></div>
}
