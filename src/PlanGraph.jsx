...
import { planLayout, turnMetrics, filesTouched } from './plan.js'
import { ParamView, ResultSection, KVTable, StatusDot, statusOf, STATUS } from './planWidgets.jsx'
    ...
    <div style={{ display: 'flex', gap: 16, alignItems: 'flex-start' }}>
      <div style={{ ...PANEL, flex: 1, overflow: 'auto', maxHeight: '70vh' }}>
        <div style={{ display: 'flex', alignItems: 'baseline', gap: 10, marginBottom: drill.length ? 6 : 12, flexWrap: 'wrap' }}>
          <div style={{ font: `600 14px ${HEAD}` }}>{drill.length ? 'Subagent graph' : derived ? 'Session activity' : 'Execution plan'} <span style={{ font: `400 11px ${MONO}`, color: '#8a807a' }}>{activeSteps.length} {useTimeline ? 'actions · grouped by turn, in execution order' : 'steps · left→right = dependency order'} · click a step for details</span></div>
          {useTimeline && <div style={{ marginLeft: 'auto', display: 'flex', gap: 6 }}>
            <button className="mini" style={{ marginTop: 0, color: lane === 'files' ? '#5eb3f6' : undefined }} onClick={() => setLane(lane === 'files' ? null : 'files')}>▤ files</button>
            <button className="mini" style={{ marginTop: 0, color: lane === 'cost' ? '#5eb3f6' : undefined }} onClick={() => setLane(lane === 'cost' ? null : 'cost')}>◫ cost</button>
          </div>}
        </div>
        {drill.length > 0 && (
        ...
        {drill.length === 0 && diagnostics && diagnostics.length > 0 && (
          <div style={{ marginBottom: 14, display: 'flex', flexDirection: 'column', gap: 6 }}>
            ...
            {diagnostics.map((d, i) => {
              const col = d.level === 'warn' ? STATUS.warn : d.level === 'good' ? STATUS.ok : '#5eb3f6'
              const ids = d.stepIds || []
              ...
              return (
                <div key={i} onClick={() => { if (active) { setHi(new Set(ids)); setSel(ids[0]); setChip(null) } }}
                  ...
                  <div style={{ flex: 1 }}>
                    <div style={{ font: `600 11px ${HEAD}`, color: '#e8ded6' }}>{d.title}{active && <span style={{ font: `500 9.5px ${MONO}`, color: col, marginLeft: 6 }}>→ {ids.length} step{ids.length > 1 ? 's' : ''}</span>}</div>
                    <div style={{ font: `400 10.5px ${MONO}`, color: '#a2988f' }}>{d.detail}</div>
                  </div>
                </div>
              )
            })}
        ...
        {useTimeline ? <ActivityTimeline steps={activeSteps} sel={sel} setSel={setSel} hi={hi} openSubgraph={openSubgraph} metrics={metrics} /> : <div style={{ position: 'relative', width: W, height: H }}>
          ...
          {activeSteps.map(s => {
            ...
            const st = statusOf(s)
            const border = isSel ? '#f6efe9' : lit ? STATUS.warn : hasSub ? 'rgba(167,139,250,0.5)' : st === 'error' ? `${STATUS.error}88` : 'rgba(255,255,255,0.12)'
            return (
              <div key={s.step_id} onClick={() => setSel(isSel ? null : s.step_id)} onDoubleClick={() => hasSub && openSubgraph(s)}
                style={{ position: 'absolute', left: p.x, top: p.y, width: CW, height: CH, boxSizing: 'border-box',
                  ...
                  background: 'rgba(20,17,15,0.9)', border: `1px solid ${border}`,
                  boxShadow: isSel ? '0 0 0 1px #f6efe9' : lit ? `0 0 0 1px ${STATUS.warn}` : 'none', transition: 'border-color .15s' }}>
                <div style={{ position: 'absolute', top: 10, right: 10 }}><StatusDot status={st} title={st === 'error' ? 'errored' : 'ok'} /></div>
                <div style={{ font: `600 11px ${HEAD}`, color: '#f6efe9', marginBottom: 4, paddingRight: 12 }}>
        ...
        <div style={{ ...PANEL, width: 340, maxHeight: '70vh', overflow: 'auto', flexShrink: 0 }}>
          {chip ? (
            ...
            <>
              ...
              <div style={{ font: `400 12px ${HEAD}`, color: '#c9bfb7', marginBottom: 10, display: 'flex', gap: 8, alignItems: 'baseline' }}>
              ...
              <div style={{ font: `600 10px ${MONO}`, color: '#a2988f', textTransform: 'uppercase', letterSpacing: 0.4, margin: '12px 0 5px' }}>Params</div>
...
const hueOf = s => s.subplan ? '#a78bfa' : (s.expected_skill && s.expected_skill.length) ? C.skill : s.mcp_server ? C.mcp : C.tool
  ...
  return (
    <>
      <div onClick={() => setSel(isSel ? null : s.step_id)}
        ...
        onMouseEnter={e => { if (!isSel && !lit) e.currentTarget.style.background = 'rgba(255,255,255,0.03)' }}
        ...
        <span title={s.expected_params?.file_path || s.description} style={{ font: `500 11px ${MONO}`, color: '#e8ded6', whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis', flex: 1 }}>{s.description || s.tool_to_call}</span>
        {hasSub && (inlineable
          ...
          : <button className="mini" style={{ marginTop: 0, padding: '1px 6px', color: '#a78bfa' }} onClick={e => { e.stopPropagation(); openSubgraph(s) }}>◆ {s.subplan.length} →</button>)}
...
const LaneHead = ({ title, sub, onClose }) => (
  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'baseline', marginBottom: 12 }}>
    <div style={{ font: `600 14px ${HEAD}`, color: '#f6efe9' }}>{title} {sub && <span style={{ font: `400 10px ${MONO}`, color: '#8a807a' }}>{sub}</span>}</div>
...
function FilesPanel({ files, onClose }) {
  return (
    <div style={LANE}>
      ...
      {files.length === 0 && <div style={{ font: `400 11px ${MONO}`, color: '#7a716a' }}>no files read or written</div>}
        ...
        {files.map(f => {
          ...
          return (
            <div key={f.path} title={f.path} style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '4px 6px', borderRadius: 6 }}>
              <span style={{ color: f.writes ? '#d97757' : '#5eb3f6', font: `600 11px ${MONO}`, flexShrink: 0 }}>{f.writes ? '✎' : '▤'}</span>
              <span style={{ font: `500 10.5px ${MONO}`, whiteSpace: 'nowrap', overflow: 'hidden', flex: 1, minWidth: 0, display: 'flex', gap: 1 }}>
                <span style={{ color: '#7a716a', overflow: 'hidden', textOverflow: 'ellipsis' }}>{dir ? '…/' + dir + '/' : ''}</span>
                <span style={{ color: '#e8ded6', flexShrink: 0 }}>{base}</span>
              </span>
              {f.reads > 0 && <span style={{ font: `400 9.5px ${MONO}`, color: '#8a807a' }}>{f.reads}r</span>}
              {f.added > 0 && <span style={{ font: `400 9.5px ${MONO}`, color: STATUS.ok }}>+{f.added}</span>}
              {f.removed > 0 && <span style={{ font: `400 9.5px ${MONO}`, color: STATUS.error }}>−{f.removed}</span>}
            </div>
          )
        })}
      </div>
...
function CostPanel({ metrics, onClose }) {
  ...
  return (
    <div style={LANE}>
      ...
      {total === 0
        ? <div style={{ font: `400 10.5px ${MONO}`, color: '#7a716a' }}>No token usage in this transcript (older sessions and live streams omit it).</div>
        : <div style={{ font: `400 10px ${MONO}`, color: '#7a716a', marginBottom: 10 }}>input + output + cache writes. {fmtTok(cached)} read from cache.</div>}
      <div style={{ display: 'flex', flexDirection: 'column', gap: 7 }}>
        {rows.map(r => (
          <div key={r.col}>
            <div style={{ display: 'flex', justifyContent: 'space-between', font: `400 10px ${MONO}`, color: '#a2988f', marginBottom: 2 }}>
              <span>Turn {r.col + 1}</span>
              <span>{r.tokens > 0 ? fmtTok(r.tokens) + ' tok' : '—'}{fmtDur(r.ms) ? ` · ${fmtDur(r.ms)}` : ''}</span>
            </div>
            <div style={{ height: 6, borderRadius: 3, background: 'rgba(255,255,255,0.06)' }}>
              <div style={{ height: '100%', borderRadius: 3, width: `${Math.round((r.tokens / maxT) * 100)}%`, background: '#5eb3f6' }} />
            </div>
          </div>
        ))}
      </div>
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
      {turns.map((band, i) => {
        ...
        return (
          <div key={i} style={{ border: '1px solid rgba(255,255,255,0.06)', borderRadius: 10, overflow: 'hidden', background: 'rgba(20,17,15,0.5)' }}>
            <div onClick={() => { const n = new Set(collapsed); n.has(i) ? n.delete(i) : n.add(i); setCollapsed(n) }}
              style={{ position: 'sticky', top: 0, zIndex: 1, display: 'flex', alignItems: 'center', gap: 8, padding: '7px 11px', cursor: 'pointer',
                background: 'rgba(28,24,21,0.92)', backdropFilter: 'blur(8px)', borderBottom: isCol ? 'none' : '1px solid rgba(255,255,255,0.05)' }}>
              <span style={{ color: '#8a807a', font: `600 10px ${MONO}` }}>{isCol ? '▸' : '▾'}</span>
              <span style={{ font: `600 11px ${HEAD}`, color: '#f6efe9' }}>Turn {band.col + 1}</span>
              <span style={{ font: `400 10px ${MONO}`, color: '#8a807a' }}>{band.rows.length} action{band.rows.length > 1 ? 's' : ''}</span>
              {m && m.tokens > 0 && <span style={{ font: `400 10px ${MONO}`, color: '#5eb3f6' }}>· {fmtTok(m.tokens)} tok</span>}
              {dur && <span style={{ font: `400 10px ${MONO}`, color: '#e0b341' }}>· {dur}</span>}
            </div>
            {!isCol && (
              <div style={{ padding: '5px 8px', display: 'flex', flexDirection: 'column', gap: 1, borderLeft: '2px solid rgba(255,255,255,0.07)', margin: '0 0 0 10px' }}>
