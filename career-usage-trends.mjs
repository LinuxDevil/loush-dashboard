...
// priceFn(model) => $ per 1M input tokens, matching this repo's PRICE_PER_M/entryCost pricing model
// (cache-write ≈ 1.25x, cache-read ≈ 0.1x that base rate).
export function cacheWasteCost(entries, priceFn, bestEff) {
  let totalRead = 0, totalCreate = 0, weightedDelta = 0, totalWeight = 0
  for (const e of entries) {
    const p = priceFn(e.model)
    const w = (e.cc || 0) + (e.cr || 0)
    weightedDelta += (p * 1.25 - p * 0.1) * w
    totalWeight += w
