...
export function buildMap(dsRepo) {
  ...
  // The repo's own package.json names the import. There is deliberately no fallback constant here:
  // a hardcoded default is one specific company's package, and it would be silently wrong — and
  // uncopyable — for every other repo. An unnamed package says so instead.
