// career-usage-trends.mjs — pure daily-trend analytics ported from oh-my-hi's
// cache-ttl.mjs / anomaly.mjs / cost-projection.mjs, adapted to this repo's entry
// shape: {t (ms), model, in, out, cc, cr, tc}. Plane B: this machine's own transcripts only.
...
// best/worst 7-day rolling cache hit efficiency, plus the daily series for a trend line
...
// $ wasted on cache-creation beyond what the best-observed efficiency period would have needed.
//
// rateFn(entry) => { write, read }, both $ per 1M tokens — pass lib/pricing.mjs's
// entryCacheRates. This used to hold its own copy of the cache ratios (`p * 1.25 - p * 0.1`)
// off a base input rate, which meant (a) the numbers here drifted from the ones in
// lib/pricing.mjs the moment either moved, and (b) 1-hour cache writes were priced at the
// 5-minute rate, understating the write side by 60%. The rates now come from the same table
// that prices everything else, per entry, so the tier and any introductory rate are already
// baked in.
//
// An unpriced model ({write:0, read:0}) is left out of the weighted average entirely rather
// than averaged in as a $0 delta — a model we hold no rate for is unknown, not free, and
// counting its tokens in the denominator would drag the average price down and quietly
// understate the waste. Its tokens still count towards the read/create volumes.
...
// flags days whose token/cost usage spikes >2x the trailing baseline (a looping-agent / runaway-session tell)
