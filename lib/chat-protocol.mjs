...
/**
 * Maps this app's own session id to the provider's, which does not exist until the CLI emits its
 * init event. Allocating ours up front is what lets a URL be shareable before the run starts.
 *
 * Rebinding to a DIFFERENT provider id is an error rather than an overwrite: two provider
 * sessions collapsing onto one app id would silently interleave two conversations.
 */
export function createSessionIndex() {
  const toProvider = new Map(), toApp = new Map()
  let n = 0
  return {
    allocate(prefix = 's') { const id = `${prefix}${(++n).toString(36)}${Date.now().toString(36).slice(-4)}`; toProvider.set(id, null); return id },
    bind(appId, providerId) {
      if (!appId || !providerId) return { ok: false, reason: 'missing-id' }
      const existing = toProvider.get(appId)
      if (existing && existing !== providerId) return { ok: false, reason: 'already-bound', existing }
      toProvider.set(appId, providerId)
      toApp.set(providerId, appId)
      return { ok: true }
    },
    providerFor: appId => toProvider.get(appId) ?? null,
    appFor: providerId => toApp.get(providerId) ?? null,
    // Rewrites a provider id to ours on the way out. A provider session id is an internal
    // identifier; leaking it lets a client address the CLI's session directly.
    toClient(frame) {
      if (!frame || typeof frame !== 'object') return frame
      const out = { ...frame }
      const pid = out.session_id ?? out.sessionId
      if (pid && toApp.has(pid)) {
        if ('session_id' in out) out.session_id = toApp.get(pid)
        if ('sessionId' in out) out.sessionId = toApp.get(pid)
      }
      return out
    },
    size: () => toProvider.size,
  }
}
