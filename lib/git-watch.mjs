...
//   .git (the DIR)      — index and HEAD are replaced by the SAME lock-and-rename dance as a ref, so
//                         the paragraph above applies to them verbatim. Measured on darwin: the
//                         `.git/index` file watch delivers exactly ONE event and is silent forever
//                         after; the `.git/HEAD` file watch delivers NONE, including across
//                         `git checkout`. The directory watch saw all of it. Without this target the
//                         dashboard reported ok:true with every watch "established" and then never
//                         noticed another stage or branch switch — the precise "looks live, is
//                         frozen" failure this module exists to prevent. The file watches are kept
//                         because inotify does deliver on them and they name the target precisely;
//                         this one is what makes the guarantee hold.
//
...
// Which direct children of .git are worth waking for. Everything else in there — objects/, logs/,
// COMMIT_EDITMSG, FETCH_HEAD — either says nothing about staged/HEAD state or is written constantly.
// A `.lock` is the ephemeral half of a rename pair; the rename onto the real name follows and is the
// event we want, so waking on both just doubles the burst the debounce has to absorb.
const GITDIR_INTEREST = /^(?:index|HEAD|packed-refs|ORIG_HEAD|MERGE_HEAD|REBASE_HEAD|CHERRY_PICK_HEAD)$/
...
export function watchRepo(dir, opts = {}) {
  ...
  const targets = [
    ...
    // The durable stand-in for the two file watches above. Non-recursive on purpose: every file we
    // care about is a direct child of .git, and a recursive watch here would also carry .git/objects
    // — thousands of writes per fetch, for no extra signal.
    { target: 'git-dir', path: gitDir, dir: true, interest: GITDIR_INTEREST },
  ...
  for (const t of targets) {
    try {
      ...
      let w
      try {
        w = fs.watch(t.path, { persistent, recursive: Boolean(t.recursive) }, (evt, name) => onFsEvent(t, evt, name))
      } catch (e) {
        if (t.recursive && (e?.code === 'ERR_FEATURE_UNAVAILABLE_ON_PLATFORM' || e?.code === 'EINVAL')) {
          w = fs.watch(t.path, { persistent }, (evt, name) => onFsEvent(t, evt, name))
  ...
  // `git-dir` substitutes for either file watch: on platforms that lose a file watch to the
// lock-and-rename it is the only one of the three that keeps delivering, so requiring the file
  // watches specifically would fail a repo that is in fact fully observable.
  const essential = (live('index') || live('git-dir')) && (live('HEAD') || live('git-dir'))
    && (live('branch-ref') || live('refs-heads-dir') || live('git-dir'))
  ...
  function onFsEvent(t, evt, name) {
    if (disposed) return
    const target = typeof t === 'string' ? t : t.target
    // `name` is null on some platforms. Filtering on a name we do not have would drop the event and
    // put us straight back to a watch that reports healthy and sees nothing, so an unnamed event
    // always gets through — settle() re-reads HEAD anyway and a spurious wake costs one `git` call.
    if (typeof t === 'object' && t.interest && name != null && !t.interest.test(name)) return
