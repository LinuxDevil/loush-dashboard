// lib/design-schema.mjs — parse, validate, merge and lay out a design graph.
//
// PURE. No I/O, no spawn, no network — so the riskiest code in the Ticket feature is also the only
// part fully verifiable without a `claude` binary or JIRA credentials. See test/lib/design-*.test.mjs.
//
// WHY YAML AND NOT JSON
// The obvious design asks the model for JSON and then fights to parse it: strip fences, brace-balance
// scan, repair round-trip. Every one of those steps exists because a single stray backslash or an
// unescaped quote inside a label kills the whole document. Design labels and notes are exactly where
// apostrophes, quotes and newlines live. YAML block scalars carry them natively, `yaml@^2.4.5` is
// ALREADY a dependency of this app, and its parser reports a line/column on failure — which is a far
// better repair hint to hand back to a model than "Unexpected token }".
//
// WHAT THE MODEL IS AND IS NOT ALLOWED TO EMIT
//   * ids are SEMANTIC SLUGS, never ordinals. src/lib/plan.js keys steps by ordinal `step_id`, which
//     is exactly why regenerating a plan there is a total rewrite with no usable diff. Slugs are the
//     merge key that lets a regeneration preserve hand edits and node positions.
//   * COORDINATES ARE REJECTED. Models cannot lay out a graph and will churn every position on every
//     run. Layout is computed here; `position` is owned by the server and the user's mouse.
...
// ---------------------------------------------------------------------------------------------
// parse — a ladder, most-likely-correct first. Never throws.
// ---------------------------------------------------------------------------------------------

/** Every fenced block, in order. The LAST valid one wins: models often show a wrong example first. */
...
export function extractDoc(raw) {
  ...
  // 1. the whole thing is already a document
  const whole = tryYaml(text)
  if (whole && (whole.nodes || whole.edges)) return { doc: whole, how: 'whole', error: null }

  // 2. fenced blocks, last valid wins, preferring an explicitly tagged fence
  ...
  // 3. the document starts at the first `nodes:` key and runs to the end of the indented region.
  //    YAML makes this tractable in a way JSON does not — no bracket matching required.
  ...
  // 4. give up, but say why — this string is fed back to the model as the repair hint.
...
// ---------------------------------------------------------------------------------------------
// validate — always returns a usable graph plus a list of what it had to change.
// ---------------------------------------------------------------------------------------------
...
export function validateGraph(doc) {
  ...
  for (const n of rawNodes) {
    ...
    // A model-supplied position is rejected outright, not merely ignored, so the rejection is
    // visible in telemetry if a future prompt starts emitting them.
    if (n.position || n.x != null || n.y != null) warn('position-rejected', `node "${id}" supplied coordinates — layout is computed, not authored`)

    nodes.push({ id, type, position: null, data: { label, note: typeof n.note === 'string' ? n.note.trim() : '', files, origin: 'generated' } })
  }
  ...
  for (const e of rawEdges) {
    ...
    edges.push({
      ...
      // `isStatic:false` means "the model asserted this", as opposed to an edge derived from a real
      // import graph. The UI renders asserted edges dashed — honest uncertainty beats a clean lie.
...
/** Convenience: raw text → `{graph, warnings, how, error}`. */
...
// ---------------------------------------------------------------------------------------------
// merge — regeneration must not destroy hand edits.
// ---------------------------------------------------------------------------------------------
...
export function mergeGraph(oldGraph, newGraph) {
  ...
  const nodes = (newGraph?.nodes || []).map(n => {
    ...
    return {
      ...n,
      position: prev.position ?? null,                       // the user's layout is theirs
      data: {
        ...n.data,
        // A hand-edited label or note is a deliberate correction of the model. Regeneration must
        // not overwrite it; the new text is kept alongside so the user can see what changed.
  ...
  // A user-authored node is not something the model can delete by omitting it.
...
// ---------------------------------------------------------------------------------------------
// layout — layered, with barycentre crossing reduction.
// ---------------------------------------------------------------------------------------------
...
export function layout(graph) {
  ...
  // Longest-path layering with an explicit visit set — a cyclic design graph must not recurse forever.
  const depth = new Map()
  const computing = new Set()
  const depthOf = id => {
    if (depth.has(id)) return depth.get(id)
    if (computing.has(id)) return 0        // cycle: treat the back-edge as depth 0
    computing.add(id)
  ...
  // Group by depth via a Map, then compact to consecutive columns.
  // `(layers[d] ||= []).push(n)` on a plain array looked equivalent and was not: when no node has
  // depth 0 — which is exactly what happens when every node has a predecessor, i.e. any cyclic
  // graph — index 0 is a HOLE. Spreading a sparse array yields `undefined` for holes, so
  // `Math.max(1, ...layers.map(l => l.length))` became NaN, every y became NaN, and JSON.stringify
  // wrote it to disk as `null`. Worse, the "already placed" guard below sees a truthy object and
  // never repairs it, so one cyclic graph corrupted that ticket's layout permanently.
  ...
  // One barycentre pass, left to right.
  const order = new Map()
  ...
  layers.forEach((layer, li) => {
    ...
    layer.forEach((n, i) => {
      if (n.position) return                                    // already placed — leave it alone
...
/** Mermaid flowchart, for export. Mermaid pastes directly into JIRA and Confluence. */
export function toMermaid(graph) {
  ...
  for (const e of graph?.edges || []) {
    const arrow = e.data?.isStatic === false ? '-.->' : '-->'    // asserted edges stay dashed in export too
