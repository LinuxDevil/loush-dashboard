...
export function validateGraph(doc, { trustPositions = false } = {}) {
  ...
  for (const n of rawNodes) {
    ...
    // A MODEL-supplied position is rejected outright, not merely ignored, so the rejection shows up
    // if a future prompt starts emitting them. A USER-supplied one is the whole point of dragging,
    // hence `trustPositions` — without the distinction every hand edit sprayed warning chips.
