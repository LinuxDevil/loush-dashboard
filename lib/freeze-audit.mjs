// Production-readiness freeze audit (feature 017).
//
// Two things live here:
//
//   1. ITEMS — the 75-item freeze-audit checklist, `FA-001`…`FA-075`, ported from
//      claude-code-build-framework (MIT, dlowenth/claude-code-build-framework, `claude.md`
//      §21 "Freeze Audit Checklist", lines 2293-2372). Item text and source order are
//      preserved verbatim; the category grouping and the `appliesTo` / `auto` / `manualReason`
//      tags are ours. The upstream list is one flat, ungrouped, untagged list.
//
//   2. auditRepo(dir) — the machine-checkable subset, run against a real checkout on disk.
//
// The whole value of (2) is that it is trustworthy, which means the failure modes matter more
// than the happy path. Four rules, all of them load-bearing:
//
//   * `manual`, `unknown` and `fail` are three different answers. `manual` means no machine
//     can decide this item — it is not a failure of the repo, and it is not a pass. `unknown`
//     means *the check itself* could not run (directory unreadable, git binary absent) and
//     always carries the reason. Neither is ever rendered as, folded into, or counted as a
//     pass or a fail. A readiness audit that reports a green it did not verify is worse than
//     no audit at all: it launders an assumption into an attestation.
//
//   * Every non-`manual` result carries `evidence` — the paths inspected, the values found,
//     the argv actually executed. A verdict without evidence is an assertion, and an assertion
//     is what the checklist already had before we automated it.
//
//   * No silent caps. The two scans that walk a tree (env-var usage, hook script contents) are
//     bounded, and every bound is reported in the evidence. If a scan is truncated the result
//     that depended on completeness degrades to `unknown` rather than claiming a pass over
//     files it never opened.
//
//   * Many checklist items are compound ("X exists **and** is correct **and** was reviewed").
//     Where we can verify one clause but not the rest, the check reports `partial`, and a
//     partial pass resolves the *item* to `manual` — never to `pass`. A failed clause is still
//     a definitive `fail`, because one broken clause breaks the item.
//
// Purity: `dir` is a parameter, nothing is read from cwd or the environment, and nothing
// touches the network. The only subprocess is git, always via spawnSync with an argv array
// (never a shell string, so no path in a repo can be interpolated into a command), and a
// missing git binary is `unknown`, never `fail`.
...
/** The only statuses any result may carry. Kept as data so a renderer can enumerate them. */
...
/** Why an item has no automated check. Only present on items where `auto` is null. */
...
/** Scan bounds. Exported because every bound that can change an answer must be inspectable. */
export const SCAN_LIMITS = Object.freeze({
  maxSourceFiles: 2000,   // env-usage scan: files opened before the walk is truncated
...
// `NODE_ENV` is set by tooling on every Node project and is never a project-configured secret;
// documenting it in .env.example is not expected. Excluded by name, and the exclusion is
// reported in the evidence rather than applied silently.
...
//
// Fields:
//   id           FA-001…FA-075, stable, matching upstream source order.
//   category     ours; the upstream list is flat.
//   text         upstream, verbatim.
//   appliesTo    null when the item is universal; otherwise the stack tags it is scoped to.
//                Tagged so a Node CLI is not shown 24 red rows about Supabase RLS policies it
//                will never have — a false fail costs the same credibility as a false pass.
//   auto         id of the check in CHECKS that (fully or partly) verifies this item, or null.
//   manualReason present only when `auto` is null; key of MANUAL_REASONS.
...
/** Categories in first-appearance order — a renderer can group without re-deriving the order. */
export const CATEGORIES = Object.freeze([...new Set(ITEMS.map(i => i.category))])

/** Every distinct stack tag used by `appliesTo`, sorted. */
// 'ccbf' tags the items that check for claude-code-build-framework's OWN file conventions —
// STATE.md, CONTEXT.md, docs/resources/README.md, lessons-learned.md, tests/role-tests.md.
// Upstream ships them untagged, which makes every repo that does not use that framework fail
// five production-readiness checks for not adopting someone else's filenames. The item text is
// still verbatim; only the scoping is ours, which is the same latitude taken for the other tags.
...
//
// Every reader distinguishes three outcomes: present, absent (ENOENT), and unreadable (anything
// else). Collapsing "unreadable" into "absent" is exactly how a permissions error turns into a
// confident red X against a repo that may be perfectly compliant.
// ---------------------------------------------------------------------------------------------
...
/** Absent → null. Unreadable → throws CheckError (which the runner converts to `unknown`). */
function readText(p) {
...
function walkSource(dir, { extensions, maxFiles, maxDepth, skipDirs }) {
  const files = []
  let truncated = false   // something was not looked at
  let stopped = false     // the file budget is exhausted; nothing more will be looked at
...
/** Evidence constructor. `detail` is prose a human can read; the rest is machine-checkable. */
const ev = (detail, extra = {}) => ({ detail, paths: [], ...extra })

// ---------------------------------------------------------------------------------------------
// The checks
//
// Each check: { id, label, itemId, run(ctx) -> { status, evidence, partial?, partialReason? } }
// `itemId` may be null — some checks report a repository fact worth surfacing that the upstream
// 75 items never covered (a LICENSE file, a detectable test command). Those are reported in
// `checks` and deliberately do NOT invent extra FA-### rows: the checklist's item set is the
// ported artifact and must stay recognisable against upstream.
// ---------------------------------------------------------------------------------------------

const RECOMMENDED_AGENTS = ['security-reviewer', 'component-checker', 'test-coverage']
const PRE_TOOL_USE_RE = /^pre[-_]?tool[-_]?use\b/i
// Patterns that indicate a hook reaches the network or shells out to fetch something. A hit is
// evidence of a real risk; the *absence* of a hit is not evidence of safety, which is why a clean
// scan returns `partial` and the item lands on `manual`.
...
/** Directory listing for a check: null when absent, throws (→ `unknown`) when unreadable. */
...
export const CHECKS = Object.freeze([
  {
    ...
    run({ dir }) {
      ...
      return {
        ...
        // The item says custom subagents exist; the parenthetical recommendation is advisory,
        // so a non-empty directory is a genuine pass and the recommended set is reported, not gated.
      }
    },
  },

  {
    ...
    run({ dir }) {
      ...
      let registered = null // null = could not determine (no/unparseable settings.json)
      if (raw != null) {
        try {
      ...
      // Script on disk but we cannot confirm it is wired up — "active" is half-verified.
      return {
...

  {
    ...
    run({ dir }) {
      ...
      let parsed
      // A settings.json that is present but invalid JSON is a determinate finding about the
      // repo (Claude Code will not load it), not a failure of this check — hence fail, not unknown.
...

  {
    ...
    run({ dir, gitBin }) {
      ...
      // No git (or not a repo): fall back to reading .gitignore directly, and say so.
...

  {
    ...
    run({ dir }) {
      ...
      const used = new Map() // key -> first "file:line" that used it
      let bytesSkipped = 0
      ...
      // A mismatch found in a partial scan is still a real mismatch, so fail survives truncation.
      if (missing.length || stale.length) return { status: 'fail', evidence }
      // A match found in a partial scan is not a real match — the missing files could hold the
      // undocumented key. Truncation must not be laundered into a green.
...

  {
    ...
    run({ dir, gitBin }) {
      ...
      if (g.status !== 0) {
        ...
        // "Not a repository" is a fact about the checkout: the item asks for a clean git repo
        // and there is none. Any other non-zero exit is a broken check, not a broken repo.
        return notRepo
...

  {
    ...
    run({ dir }) {
      ...
      if (!found.length && !dep) {
        // The item is explicitly "if applicable". No config and no dependency means it is not.
...

  {
    ...
    run({ dir }) {
      ...
      // Whether noindex is *correct* depends on whether public indexing was authorised, which is
      // a project decision. We report exactly what is in the file and let a human rule.
      return {
        status: 'manual',
  ...
  // Reported because they are cheap, honest and useful; deliberately not given FA-### ids, so
  // the ported checklist stays byte-comparable with upstream's 75.
  {
    id: 'license-file',
...

  {
    ...
    itemId: null,
    run({ dir }) {
      // Delegated to lib/testdetect.mjs, which already owns marker→command detection for a
      // dozen ecosystems and, crucially, already returns null rather than guessing.
      let detected
...
/** Shared body for the "this doc must exist" checks — presence is checkable, contents are not. */
...
/** Keys declared in a .env.example body. `export FOO=` and `FOO=` both count; comments do not. */
...
export function auditRepo(dir, opts = {}) {
  ...
  // Preflight. If the root itself is not a readable directory, every per-item verdict would be
  // an artefact of that one failure, so we say `unknown` across the board once and say why.
  ...
  if (typeof dir !== 'string' || !dir) preflightError = 'no directory was given'
    ...
    try {
      ...
      else fs.readdirSync(dir) // surfaces EACCES here rather than inside 18 separate checks
    } catch (err) {
  ...
  const items = ITEMS.map(item => {
    ...
    if (!r) {
      // No check at all: manual, with the reason it is manual. Evidence is null by design —
      // asserting evidence here would mean claiming we looked at something. We did not.
      return { ...base, status: 'manual', evidence: null }
    }
    if (r.status === 'pass' && r.partial) {
      // The decisive rule: a half-verified item is not a pass. It carries the evidence we do
      // have plus the clause we could not settle, and lands on the human.
      return {
        ...base,
        status: 'manual',
  ...
  // A human tick is recorded alongside the machine verdict, never on top of it: `tickedByHuman`
  // can lift a `manual` out of the blocker list, and can do nothing whatsoever to a `fail` or an
  // `unknown`. Attestation cannot overrule observation.

  const summary = summarise(items)
  // Upstream's verdict token. `unknown` blocks just as hard as `fail`: not knowing whether the
  // working tree is clean is not the same as knowing it is.
