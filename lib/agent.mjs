...
/**
 * `disallowedTools` exists for callers whose contract is "give me text, touch nothing" — the
 * Documents AI edit is the first. Skipping permissions is right for a build agent and wrong for a
 * pure transform: without it, an endpoint's write-free return path guarantees nothing, because the
 * model can pick up Write and edit the file itself. Refusing the tools is the only place that
 * promise can be enforced rather than merely requested in the prompt.
 */
export function runAgent({ cwd, prompt, model, timeoutMs = 1800_000, resume, disallowedTools }) {
