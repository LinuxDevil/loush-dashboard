 ...
 */
export function runAgent({ cwd, prompt, model, timeoutMs = 1800_000, resume, allowedTools }) {
...
export function spawnAgent({ cwd, prompt, model, resume, timeoutMs = 1800_000, onEvent, onExit }) {
  ...
  const finish = payload => {
    ...
    // Same failure, a different way in. A child killed by a SIGNAL exits with `code === null`, so a
    // caller folding only the code — `code ? ... : null` — got `{code: null, error: null}` and could
    // not tell an OOM kill, a `pkill claude`, or a Ctrl-C that hit the whole process group from a
    // clean finish. `timedOut` does not cover it either: that flag is only set by the timer below.
    // Both callers treat a null error as success and act on it (server/research.mjs publishes the
    // run as `done`; server/ticket.mjs renames the STAGED document into place), so the fix belongs
    // here, once, rather than in each caller's idea of what a clean exit looks like.
    else if (payload?.signal && !payload?.error) payload = { ...payload, error: `the agent process was killed (${payload.signal})` }
    onExit?.(payload)
  }
