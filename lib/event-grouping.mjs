...
export function groupEvents(records, opts = {}) {
  ...
  const counts = {
    ...
    groups: 0,
    malformed: 0,
    // Sidechain (subagent) events, and how many of them carried no parent tool_use id we could
    // resolve. Real subagent files often carry only `agentId`, so a caller that wants nesting for
    // those has to supply the association itself — reported rather than silently flattened.
    sidechain: 0,
    sidechainUnlinked: 0,
    // Record `type`s this build does not model (e.g. 'attachment'), counted by type so a caller can
    // see that something was passed over instead of assuming the timeline is complete.
    skippedRecordTypes: {},
  }
