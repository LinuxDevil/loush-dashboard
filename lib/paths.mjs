...
/** Path for one ticket's design state. The key is validated by the caller; this also hard-guards it. */
export const ticketStateFile = key => {
  const safe = String(key || '').toUpperCase().replace(/[^A-Z0-9-]/g, '')
  if (!safe) throw new Error('invalid ticket key')
  return path.join(TICKET_DIR, `${safe}.json`)
}
