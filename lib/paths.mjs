...
/** Directory holding one project's saved tickets. */
export const ticketProjectDir = project => path.join(TICKET_DIR, safeSeg(project, 'project'))
 ...
 */
export const ticketStateFile = (project, key) => path.join(ticketProjectDir(project), `${safeSeg(key, 'ticket key')}.json`)

/** Pre-partition layout, read-only, so state saved before this change is not orphaned. */
export const legacyTicketStateFile = key => path.join(TICKET_DIR, `${safeSeg(key, 'ticket key')}.json`)
