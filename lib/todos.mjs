...
export function normalizeTodo(input = {}, now = Date.now()) {
  ...
  return {
    ...
    // A JIRA key, when the todo is the local half of a ticket. Stored as {key, url, host}: the URL is
    // resolved ONCE, on the server, from the configured host — a client that builds its own link
    // guesses the host, and a guessed link that 404s is worse than a plain key.
    ...
    // Roll-over bookkeeping. `firstDate` is the day this was ORIGINALLY filed for and never moves, so
    // "carried for 4 days" stays true no matter how many times the date advanced.
    firstDate: date,
    rollovers: 0,
    createdAt: now,
    updatedAt: now,
    // Stage transitions, plus roll-over events tagged `kind:'rollover'`. Anything without a stage
    // `to` is skipped by the time-in-stage walk — a date change is not a stage change.
...
export function applyPatch(todo, patch = {}, now = Date.now()) {
  ...
  // `jira: null` unlinks. The key itself is normalized by the caller (the server reuses the Ticket
  // section's normalizeKey, which already accepts every form a human pastes, including a browse URL).
  ...
  if (t.firstDate === undefined) t.firstDate = todo.date || t.date   // records written before roll-over existed
...
// ---------------------------------------------------------------------------
// roll-over — unfinished work follows you to the next day
// ---------------------------------------------------------------------------
...
/** How long a todo has been outstanding, in whole days since it was first filed. 0 = filed today. */
...
export function stageIntervals(todo, now = Date.now()) {
  ...
  const out = []
  for (const h of hist) {
    if (h === first || !isStatus(h.to)) continue // roll-overs and other non-stage events do not split the timeline
...
/** Card-level view: per-stage time, where it is now and for how long, and end-to-end lead time. */
export function timeInStages(todo, now = Date.now()) {
  ...
  return {
    byStatus,
    total,
    // Ordered for rendering: pipeline order, only the stages that actually accrued time.
    ...
    // Lead time is only meaningful once it is finished; `null` rather than "so far" so an average
    // over lead times is an average over completed work, not a mix of the two.
    leadMs: doneAt && created != null ? doneAt - created : null,
    ageMs: created != null ? (doneAt || now) - created : null,
  }
}

/** ms → "3d 4h" / "2h 10m" / "45s". Compact enough for a card, exact enough to act on. */
...
/** ISO week: Monday-start. Sunday is the END of a week, which `getDay() === 0` gets wrong by six days. */
...
export function insights(todos = [], { period = 'day', date = dayKey(), now = Date.now(), minSample = 2 } = {}) {
  ...
  // stage time, clipped to the window
  ...
  // per-day series across the window (hours for a single day) — created vs completed
  const buckets = []
  if (range.period === 'day') {
    for (let h = 0; h < 24; h++) {
  ...
  // roll-over pressure: how much work is being pushed forward rather than finished
  const rolls = todos.reduce((n, t) => n + (t.history || []).filter(h => h.kind === 'rollover' && h.at >= from && h.at < to).length, 0)
  // Keyed on how far the todo has travelled from the day it was FILED for, not on the roll-over
  // counter: pulling a todo forward by hand carries it exactly as much as the automatic pass does,
  // and counting only the automatic moves would hide the work someone is quietly re-dating each day.
  ...
  return {
    ...
    // An empty window is stated, not drawn as a flat chart that looks like a real zero.
