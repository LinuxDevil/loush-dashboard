...
export function lintPermissions(perms, ctx = {}) {
  ...
  for (const list of ['allow', 'ask', 'deny']) {
    ...
    v.forEach((rule, i) => {
      ...
      // `[A-Za-z]+` alone rejected every MCP rule: `mcp__plugin_x__do_thing` is the documented
      // form for permitting an MCP tool, and there were eight of them in one real settings.local.json.
      // Calling a valid rule malformed is worse than missing a bad one — the fix text tells the
      // user to rewrite a rule that already works.
      else if (!/^[A-Za-z][A-Za-z0-9_-]*(\(.*\))?$/.test(rule.trim()))
        out.push(diag('perm/malformed-rule', SEVERITY.WARN, `\`permissions.${list}[${i}]\` = \`${rule}\` is not in \`Tool\`, \`Tool(pattern)\` or \`mcp__server__tool\` form. Rules are matched by exact string equality against that shape, so a rule in any other form silently never applies. Fix: rewrite it, e.g. \`Bash(git push:*)\` or \`Read(//path/**)\`.`, { file, ...loc(src, list), evidence: rule.slice(0, 120) }))
    })
  }
